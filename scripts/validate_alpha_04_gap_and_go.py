"""
Ashva Quantitative Alpha 04 Validation Engine: Gap & Go (alpha_04_gap_and_go)
Executes the institutional research lifecycle:
1. Ingests clean Angel One SmartAPI 15m candles across 11 liquid NSE stocks.
2. Backtests alpha_04_gap_and_go with Risk-Budget Sizing and exact Indian Regulatory Costs.
...
def run_gap_and_go_validation():
    print("=" * 95)
    print("[*] ASHVA RESEARCH LAB: ALPHA 04 (GAP & GO) INSTITUTIONAL VALIDATION")
    print("=" * 95)
    ...
    for sym in universe:
        try:
            ...
            res = engine.run(
                signals_df,
                symbol=sym,
                strategy_id="alpha_04_gap_and_go",
    ...
    # -------------------------------------------------------------------------
    best_sym = max(results_table, key=lambda x: x["net_pnl"])["symbol"] if results_table else "RELIANCE"
    for sym in ["RELIANCE", "TCS", "INFY", best_sym]:
        if sym in valid_dfs:
            sig = strat.generate_signals(valid_dfs[sym])
            r = engine.run(sig, symbol=sym, strategy_id="alpha_04_gap_and_go", risk_per_trade_pct=0.005, capital_per_trade_pct=0.25)
            ts_path = tearsheet_gen.generate_html_tearsheet(r)
            print(f"[+] HTML Tearsheet Saved: {ts_path}")

    target_candidate = best_sym
    ...
    stress_matrix = engine.run_slippage_stress_matrix(candidate_signals, symbol=target_candidate, strategy_id="alpha_04_gap_and_go")
    pd.set_option('display.max_columns', None)
    pd.set_option('display.width', 1000)
    print(stress_matrix.to_string(index=False))

    # -------------------------------------------------------------------------
    # STEP 3: 4-GATE STATISTICAL VALIDATION (CANDIDATE: TOP ASSET)
    # -------------------------------------------------------------------------
