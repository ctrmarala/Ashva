"""
Ashva Quantitative Alpha 01 Validation Engine: TrendSurfer Pro
Executes the institutional research lifecycle:
1. Ingests clean Angel One SmartAPI 15m candles across 11 liquid NSE stocks.
2. Backtests TrendSurfer Pro with Risk-Budget Sizing and exact Indian Regulatory Costs.
...
def run_trend_surfer_validation():
    print("=" * 95)
    print("[*] ASHVA RESEARCH LAB: ALPHA 01 (TRENDSURFER PRO) INSTITUTIONAL VALIDATION")
    print("=" * 95)
    ...
    print(f"    - Risk Budget per Trade: 0.50% (Rs 2,500) | Sizing: Quantity = Rs 2,500 / (1.5 * ATR)")
    ...
    # -------------------------------------------------------------------------
    # STEP 2: 5-TIER SLIPPAGE STRESS MATRIX ON TOP ASSET
    # -------------------------------------------------------------------------
    if valid_dfs:
        best_sym = max(results_table, key=lambda x: x["net_pnl"])["symbol"] if results_table else list(valid_dfs.keys())[0]
        print(f"\n[+] STEP 2: 5-TIER SLIPPAGE STRESS TESTING (TOP ASSET: {best_sym})")
        top_signals = strat.generate_signals(valid_dfs[best_sym])
        stress_matrix = engine.run_slippage_stress_matrix(top_signals, symbol=best_sym, strategy_id="TrendSurfer_Pro")
    ...
    if "INFY" in valid_dfs:
        val_report = validator.validate_hypothesis(strat, valid_dfs["INFY"])
