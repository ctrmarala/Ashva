"""
Ashva Quantitative Strategy: Linear Regression Trend Slope Deep Compression Spring (Alpha 82)
Category: LINEAR_REGRESSION_TREND_SPRING
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity has a positive 20-day statistical linear regression slope (LinReg_Slope > 0 for Long,
or LinReg_Slope < 0 for Short) aligned with its 50 SMA and exhibits ultra-tight daily volatility compression
(Range[T-1] <= 0.70x 14-day ATR), institutional accumulation is wound up tightly in the direction of the trend.
...
class Alpha82LinearRegressionTrendSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "82_alpha"
    hypothesis_id = "82_alpha"
    name = "82_alpha — Linear Regression Trend Slope Compression Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="LINEAR_REGRESSION_TREND_SPRING",
            economic_rationale=(
                "Statistical linear regression slope combined with sub-0.70x ATR daily volatility compression "
        ...
        BaseStrategy.__init__(self, strategy_id="82_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 20-day slope: (Close[T-1] - Close[T-20]) / 20.0
        slope20 = (c.shift(1) - c.shift(20)) / 20.0
        ...
        is_bull_trend = (c1 > sma50) & (slope20 > 0) & is_orderly
        is_bear_trend = (c1 < sma50) & (slope20 < 0) & is_orderly
        ...
        for i in range(1, n):
            ...
            if t_str == "09:15" and not traded_today:
                ...
                if bar_rng > 0:
                    # BULLISH LINEAR REGRESSION SPRING:
                    ...
                    # BEARISH LINEAR REGRESSION SPRING:
