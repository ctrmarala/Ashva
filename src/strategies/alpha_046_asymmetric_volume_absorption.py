"""
Ashva Quantitative Strategy: Asymmetric Volume Absorption Spring (Alpha 46)
Category: VOLUME_PRICE_ABSORPTION_SPRING
Market Mechanism: BREAKOUT

Hypothesis:
When an equity experiences high volume on Day T-1 (Volume >= 1.15x 10-day SMA) but an ultra-compressed range
(Range < 0.80x 10-day ATR), institutional limit-order absorption is occurring. When the Day T opening 15m candle
breaks out beyond Day T-1's boundary with RVOL >= 1.20x and a decisive directional body (>= 0.60),
the accumulated inventory unleashes an aggressive intraday trend expansion.
"""
...
class Alpha46AsymmetricVolumeAbsorption(BaseHypothesis, BaseStrategy):
    strategy_id = "46_alpha"
    hypothesis_id = "46_alpha"
    name = "46_alpha — Asymmetric Volume Absorption Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="VOLUME_PRICE_ABSORPTION_SPRING",
            economic_rationale=(
                "Volume-price divergence on Day T-1 (high volume, narrow range) indicates institutional supply absorption. "
                "The opening volume drive triggers immediate expansion in the direction of the absorption."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="46_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 10-day ATR and 10-day Volume SMA
        atr10 = rng.shift(2).rolling(10, min_periods=8).mean()
        vol_sma10 = v.shift(2).rolling(10, min_periods=8).mean()

        # Absorption condition: Elevated Volume (>= 1.15x) + Compressed Range (< 0.80x ATR)
        is_high_vol = v.shift(1) >= (1.15 * vol_sma10)
        is_compressed_rng = rng.shift(1) < (0.80 * atr10)
        is_absorption = is_high_vol & is_compressed_rng

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_absorption"] = pd.Series(dates, index=out.index).map(is_absorption).ffill().fillna(False)
        ...
        absorption_flags = out["is_absorption"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if absorption_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
