"""
Ashva Quantitative Strategy: 4-Day Trend Momentum Continuation Surge (Alpha 83)
Category: MULTI_DAY_TREND_SURGE
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity exhibits a verified 4-day directional close trend (Close[T-1] > Close[T-2] > Close[T-3] > Close[T-4] for Long,
or Close[T-1] < Close[T-2] < Close[T-3] < Close[T-4] for Short) with orderly volatility (Range[T-1] <= 0.75x ATR) and alignment with 50 SMA,
institutional accumulation continues into Day T.
...
class Alpha83FourDayTrendMomentumSurge(BaseHypothesis, BaseStrategy):
    strategy_id = "83_alpha"
    hypothesis_id = "83_alpha"
    name = "83_alpha - 4-Day Trend Momentum Continuation Surge"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="MULTI_DAY_TREND_SURGE",
            economic_rationale=(
                "Consecutive 4-day directional close accumulation with controlled range (<= 0.75x ATR) and 50 SMA alignment "
                "creates strong institutional momentum continuation on Day T morning breakouts, independent of inside-day compression."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="83_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_4d_bull = (c1 > c2) & (c2 > c3) & (c3 > c4) & (c1 > sma50)
        is_4d_bear = (c1 < c2) & (c2 < c3) & (c3 < c4) & (c1 < sma50)
        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.75))
        is_orderly = (rng1 <= (atr14 * max_rng_ratio))

        is_bull_trend_map = (is_4d_bull & is_orderly).to_dict()
        is_bear_trend_map = (is_4d_bear & is_orderly).to_dict()
