"""
Ashva Quantitative Strategy: CLV Accumulation Thrust (Alpha 97)
Category: CLV_ACCUMULATION_THRUST
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When an equity exhibits institutional closing accumulation on Day T-1 with a Close Location Value
CLV = ((Close - Low) - (High - Close)) / (High - Low) >= +0.60 (settling in top 20% of range for Long, or <= -0.60 for Short)
within an orderly daily range (Range[T-1] <= 0.70x ATR) and trend alignment (Close[T-1] > 20 EMA > 50 SMA for Long, or Close[T-1] < 20 EMA < 50 SMA for Short),
institutions have dominated the daily closing auction without triggering widespread retail awareness.
A confirming morning opening breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x triggers an asymmetric continuation surge.
"""
...
class Alpha97ClvAccumulationThrust(BaseHypothesis, BaseStrategy):
    strategy_id = "97_alpha"
    hypothesis_id = "97_alpha"
    name = "97_alpha - CLV Accumulation Thrust"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="CLV_ACCUMULATION_THRUST",
            economic_rationale=(
                "Settling near the absolute high of the day (CLV >= +0.60) within a tight daily range (<= 0.70x ATR) "
                "demonstrates institutional closing dominance, setting up high-probability continuation on Day T morning breakouts."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="97_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Close Location Value: ((c1 - l1) - (h1 - c1)) / max(rng1, 1e-4)
        clv1 = np.where(rng1 > 0, ((c1 - l1) - (h1 - c1)) / rng1, 0.0)
        clv_series = pd.Series(clv1, index=daily_summary.index)

        min_clv = float(self.parameters.get("min_clv", 0.60))
        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.70))

        is_bull_setup = (clv_series >= min_clv) & (c1 > ema20) & (ema20 > sma50) & (rng1 <= (atr14 * max_rng_ratio))
        is_bear_setup = (clv_series <= -min_clv) & (c1 < ema20) & (ema20 < sma50) & (rng1 <= (atr14 * max_rng_ratio))
