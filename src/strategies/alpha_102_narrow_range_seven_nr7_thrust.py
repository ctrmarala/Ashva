"""
Ashva Quantitative Strategy: Narrow Range 7 (NR7) Volatility Expansion Thrust (Alpha 102)
Category: NR7_VOLATILITY_EXPANSION
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When an equity exhibits an NR7 day on Day T-1 (where Day T-1 range is the narrowest among the past 7 sessions)
with range compression (Range[T-1] <= 0.70x ATR) aligned with the primary 50 SMA trend (Close[T-1] > 50 SMA for Long, or < 50 SMA for Short),
volatility cycle theory predicts an imminent regime transition from extreme contraction to powerful expansion.
A confirming morning opening breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x triggers an explosive directional thrust.
"""
...
class Alpha102NarrowRangeSevenNR7Thrust(BaseHypothesis, BaseStrategy):
    strategy_id = "102_alpha"
    hypothesis_id = "102_alpha"
    name = "102_alpha - Narrow Range 7 (NR7) Volatility Expansion Thrust"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="NR7_VOLATILITY_EXPANSION",
            economic_rationale=(
                "NR7 volatility compression (narrowest range in 7 days) within <= 0.70x ATR indicates total supply exhaustion "
                "before an asymmetric morning breakout impulse in the 50 SMA trend direction."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="102_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        min_rng7 = rng.shift(1).rolling(7, min_periods=7).min()
        ...
        is_nr7 = (rng1 <= (min_rng7 + 1e-4)) & (rng1 <= (atr14 * max_rng_ratio))
        is_bull_setup = is_nr7 & (c1 > sma50)
        is_bear_setup = is_nr7 & (c1 < sma50)
