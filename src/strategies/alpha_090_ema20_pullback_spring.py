"""
Ashva Quantitative Strategy: 20 EMA Dynamic Trend-Pullback Spring (Alpha 90)
Category: DYNAMIC_TREND_PULLBACK_SPRING
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity in a confirmed 50 SMA macro trend (Close[T-1] > 50 SMA for Long, or < 50 SMA for Short)
tests and respects its 20-day Exponential Moving Average (Low[T-1] <= EMA20 and Close[T-1] >= EMA20 for Long,
or High[T-1] >= EMA20 and Close[T-1] <= EMA20 for Short) on compressed daily range (Range[T-1] <= 0.70x ATR),
institutional trend-followers defend the dynamic average.
A confirming morning directional breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x triggers sharp continuation off the support zone.
"""
...
class Alpha90EMA20PullbackSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "90_alpha"
    hypothesis_id = "90_alpha"
    name = "90_alpha - 20 EMA Dynamic Trend-Pullback Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="DYNAMIC_TREND_PULLBACK_SPRING",
            economic_rationale=(
                "Testing and bouncing off the dynamic 20 EMA on compressed range (<= 0.70x ATR) within a 50 SMA trend "
                "allows low-risk, high-expectancy trend resumption on Day T morning gap breakouts."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="90_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 20 EMA touch and close respect
        is_ema_bull = (l1 <= ema20) & (c1 >= ema20) & (c1 > sma50)
        is_ema_bear = (h1 >= ema20) & (c1 <= ema20) & (c1 < sma50)
        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.70))
        is_orderly = (rng1 <= (atr14 * max_rng_ratio))

        is_bull_trend_map = (is_ema_bull & is_orderly).to_dict()
        is_bear_trend_map = (is_ema_bear & is_orderly).to_dict()
