"""
Ashva Quantitative Strategy: Macro 100 SMA Trend Compression Spring (Alpha 62)
Category: MACRO_TREND_MOMENTUM
Market Mechanism: BREAKOUT

Hypothesis:
When an equity is aligned with the multi-month macro trend (Close > 100 SMA and Close > 20 EMA for Long, 
or Close < 100 SMA and Close < 20 EMA for Short) and coils tightly (Daily Range <= 0.75x 14-day ATR),
morning opening directional drives deliver reliable institutional expansion.
"""
...
class Alpha62MacroTrendCompressionSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "62_alpha"
    hypothesis_id = "62_alpha"
    name = "62_alpha — Macro 100 SMA Trend Compression Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="MACRO_TREND_MOMENTUM",
            economic_rationale=(
                "Macro 100 SMA trend alignment with 20 EMA and sub-0.75x ATR coiling "
                "filters institutional momentum continuation."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="62_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        sma100 = c.shift(2).rolling(100, min_periods=40).mean()
        ema20 = c.shift(2).ewm(span=20, adjust=False).mean()

        is_compressed = rng.shift(1) <= (0.75 * atr14)
        bull_trend_spring = (c.shift(1) > sma100) & (c.shift(1) > ema20) & is_compressed
        bear_trend_spring = (c.shift(1) < sma100) & (c.shift(1) < ema20) & is_compressed
