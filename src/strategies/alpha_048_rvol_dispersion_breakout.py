"""
Ashva Quantitative Strategy: Multi-Day Relative Volume Dispersion Breakout (Alpha 48)
Category: VOLUME_DISPERSION_VOLATILITY_EXPANSION
Market Mechanism: BREAKOUT

Hypothesis:
When an equity experiences range compression on Day T-1 (Range < 0.85x 10-day ATR) and the opening 15m candle on Day T
prints an anomalous institutional volume shock (RVOL >= 1.30x time-of-day mean) with directional price expansion beyond Day T-1,
the sudden liquidity injection drives sustained intraday trend continuation with high post-tax net profit factor.
"""
...
class Alpha48RVOLDispersionBreakout(BaseHypothesis, BaseStrategy):
    strategy_id = "48_alpha"
    hypothesis_id = "48_alpha"
    name = "48_alpha — RVOL Dispersion Breakout"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="VOLUME_DISPERSION_VOLATILITY_EXPANSION",
            economic_rationale=(
                "Day T-1 range compression followed by Day T opening RVOL >= 1.30x volume surge captures "
                "institutional order flow imbalances breaking out of equilibrium into rapid trend expansion."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="48_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_compressed = rng.shift(1) < (0.85 * atr10)

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_compressed"] = pd.Series(dates, index=out.index).map(is_compressed).ffill().fillna(False)
        ...
        comp_flags = out["is_compressed"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if comp_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
