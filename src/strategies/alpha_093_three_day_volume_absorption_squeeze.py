"""
Ashva Quantitative Strategy: 3-Day Volume Absorption Squeeze Surge (Alpha 93)
Category: VOLUME_ABSORPTION_SQUEEZE
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When an equity exhibits 3 consecutive days of rising daily volume (Vol[T-1] > Vol[T-2] > Vol[T-3])
accompanied by contracting price range (Range[T-1] < Range[T-2] < Range[T-3]) in alignment with the 50 SMA
(Close[T-1] > 50 SMA for Long, or < 50 SMA for Short), institutional Wyckoff absorption is taking place.
A confirming morning directional breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x triggers immediate high-velocity expansion.
"""
...
class Alpha93ThreeDayVolumeAbsorptionSqueeze(BaseHypothesis, BaseStrategy):
    strategy_id = "93_alpha"
    hypothesis_id = "93_alpha"
    name = "93_alpha - 3-Day Volume Absorption Squeeze Surge"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="VOLUME_ABSORPTION_SQUEEZE",
            economic_rationale=(
                "Rising volume with contracting range over 3 sessions reveals institutional absorption coiling "
                "within a 50 SMA trend, which unleashes powerful continuation on Day T morning gap breakouts."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="93_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 3-day volume absorption: Volume rising while range contracts
        is_vol_rising = (v1 > v2) & (v2 > v3)
        is_rng_contracting = (rng1 < rng2) & (rng2 < rng3)
        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.75))
        is_orderly = (rng1 <= (atr14 * max_rng_ratio))

        is_bull_squeeze = is_vol_rising & is_rng_contracting & (c1 > sma50) & is_orderly
        is_bear_squeeze = is_vol_rising & is_rng_contracting & (c1 < sma50) & is_orderly

        is_bull_trend_map = is_bull_squeeze.to_dict()
        is_bear_trend_map = is_bear_squeeze.to_dict()
