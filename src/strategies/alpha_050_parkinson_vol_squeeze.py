"""
Ashva Quantitative Strategy: Historical Volatility Squeeze with Volume Surge (Alpha 50)
Category: PARKINSON_VOLATILITY_SQUEEZE
Market Mechanism: BREAKOUT

Hypothesis:
When an equity's 10-day Parkinson Volatility drops into its lowest 20th percentile (extreme multi-week compression)
coinciding with an Inside Day on Day T-1, stored potential energy reaches a maximum.
An opening 15m impulse on Day T with RVOL >= 1.20x and strong directional candle body (>= 0.60)
triggers rapid trend expansion with exceptional risk-adjusted returns and robust post-tax profit factor.
"""
...
class Alpha50ParkinsonVolSqueeze(BaseHypothesis, BaseStrategy):
    strategy_id = "50_alpha"
    hypothesis_id = "50_alpha"
    name = "50_alpha — Parkinson Volatility Squeeze"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="PARKINSON_VOLATILITY_SQUEEZE",
            economic_rationale=(
                "Parkinson volatility multi-week low compression combined with Inside Day coiling captures extreme equilibrium. "
                "The subsequent opening volume catalyst triggers explosive directional trend continuation."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="50_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Parkinson Historical Volatility (10-day)
        # term = (ln(H/L))^2 / (4 * ln(2))
        log_hl_sq = (np.log(np.maximum(1e-4, h / np.maximum(1e-4, l)))) ** 2
        parkinson_var = log_hl_sq.rolling(10, min_periods=8).mean() / (4.0 * np.log(2.0))
        parkinson_vol = np.sqrt(np.maximum(1e-8, parkinson_var))

        # 50-day rolling 25th percentile of Parkinson Volatility
        vol_q25 = parkinson_vol.shift(2).rolling(50, min_periods=30).quantile(0.25)
        is_vol_squeeze = parkinson_vol.shift(1) <= vol_q25

        # Inside Day on Day T-1
        is_id = (h.shift(1) < h.shift(2)) & (l.shift(1) > l.shift(2))
        is_parkinson_id = is_id & is_vol_squeeze

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_parkinson_id"] = pd.Series(dates, index=out.index).map(is_parkinson_id).ffill().fillna(False)
        ...
        parkinson_flags = out["is_parkinson_id"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if parkinson_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
