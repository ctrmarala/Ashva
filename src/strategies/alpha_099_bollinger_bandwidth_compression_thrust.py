"""
Ashva Quantitative Strategy: Bollinger Bandwidth Compression Thrust (Alpha 99)
Category: BOLLINGER_BANDWIDTH_COMPRESSION
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When daily Bollinger Bandwidth (4 * StdDev20 / SMA20) contracts into an extreme volatility squeeze (Bandwidth[T-1] <= 0.08)
coinciding with an ultra-tight daily bar (Range[T-1] <= 0.70x ATR) and trend alignment (Close[T-1] > SMA20 > SMA50 for Long,
or Close[T-1] < SMA20 < SMA50 for Short), market volatility is at historical lows and ready for explosive expansion.
A confirming morning opening breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x unleashes the directional breakout impulse.
"""
...
class Alpha99BollingerBandwidthCompressionThrust(BaseHypothesis, BaseStrategy):
    strategy_id = "99_alpha"
    hypothesis_id = "99_alpha"
    name = "99_alpha - Bollinger Bandwidth Compression Thrust"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="BOLLINGER_BANDWIDTH_COMPRESSION",
            economic_rationale=(
                "Prolonged multi-day Bollinger Bandwidth compression followed by tight daily range (<= 0.70x ATR) "
                "identifies low-volatility coiling before massive institutional breakout expansion."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="99_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        sma20 = c.shift(1).rolling(20, min_periods=10).mean()
        std20 = c.shift(1).rolling(20, min_periods=10).std()
        bandwidth = (4.0 * std20) / np.maximum(sma20, 1e-4)
        ...
        max_bw = float(self.parameters.get("max_bandwidth", 0.08))
        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.70))

        is_bull_setup = (bandwidth <= max_bw) & (c1 > sma20) & (sma20 > sma50) & (rng1 <= (atr14 * max_rng_ratio))
        is_bear_setup = (bandwidth <= max_bw) & (c1 < sma20) & (sma20 < sma50) & (rng1 <= (atr14 * max_rng_ratio))
