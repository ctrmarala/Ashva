"""
Ashva Quantitative Strategy: Inside Day Volume Dry-Up Spring (Alpha 53)
Category: VOLUME_DRYUP_VOLATILITY_SPRING
Market Mechanism: BREAKOUT

Hypothesis:
When an equity forms an Inside Day on Day T-1 while volume drops below 85% of its 10-day SMA,
market order flow experiences complete pre-expansion liquidity dry-up.
An opening 15m breakout on Day T with institutional relative volume shock (RVOL >= 1.20x) and strong directional candle body (>= 0.60)
triggers rapid trend expansion across liquid equities with high post-tax net profit factor.
"""
...
class Alpha53InsideDayVolumeDryUpSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "53_alpha"
    hypothesis_id = "53_alpha"
    name = "53_alpha — Inside Day Volume Dry-Up Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="VOLUME_DRYUP_VOLATILITY_SPRING",
            economic_rationale=(
                "Inside Day coinciding with sub-0.85x 10-day volume dry-up represents severe supply exhaustion. "
                "The subsequent opening volume shock triggers rapid directional follow-through."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="53_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_vol_dryup = v.shift(1) < (0.85 * vol_sma10)

        # Inside Day on Day T-1: H(T-1) < H(T-2) AND L(T-1) > L(T-2)
        is_id = (h.shift(1) < h.shift(2)) & (l.shift(1) > l.shift(2))
        is_dryup_spring = is_id & is_vol_dryup

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_dryup_spring"] = pd.Series(dates, index=out.index).map(is_dryup_spring).ffill().fillna(False)
        ...
        dryup_flags = out["is_dryup_spring"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if dryup_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
