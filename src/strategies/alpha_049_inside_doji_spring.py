"""
Ashva Quantitative Strategy: Inside Day Narrow-Body Doji Spring (Alpha 49)
Category: INSIDE_DOJI_VOLATILITY_EXPANSION
Market Mechanism: BREAKOUT

Hypothesis:
When an equity experiences an Inside Day with a narrow body (Body < 0.35x of Range - an Inside Doji),
market balance reaches extreme equilibrium. An opening 15m breakout on Day T with relative volume shock
(RVOL >= 1.20x) and decisive candle body (>= 0.60) produces rapid directional momentum expansion with high post-tax net profit factor.
"""
...
class Alpha49InsideDojiSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "49_alpha"
    hypothesis_id = "49_alpha"
    name = "49_alpha — Inside Day Narrow-Body Doji Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="INSIDE_DOJI_VOLATILITY_EXPANSION",
            economic_rationale=(
                "Inside Day with a compressed real body represents complete auction equilibrium. "
        ...
        BaseStrategy.__init__(self, strategy_id="49_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Narrow Body on Day T-1: Body < 0.35x Range
        is_narrow_body = body.shift(1) < (0.35 * rng.shift(1))
        is_id_doji = is_id & is_narrow_body

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_id_doji"] = pd.Series(dates, index=out.index).map(is_id_doji).ffill().fillna(False)
        ...
        id_doji_flags = out["is_id_doji"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if id_doji_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
