"""
Ashva Quantitative Strategy: Trend-Aligned Dual Compression (Alpha 42)
Category: MULTI_DAY_TREND_VOLATILITY_COMPRESSION
Market Mechanism: BREAKOUT

Hypothesis:
When an equity in an established intermediate trend (Daily EMA 20 vs EMA 50) forms a Dual Compression pattern
(Day T-1 is both an Inside Day AND an NR4 - Narrowest Range of 4 days), volatility coils to an extreme.
An opening impulse on Day T with RVOL >= 1.25x and directional alignment with the daily trend unleashes
high-velocity trend-continuation expansion with robust post-tax profit factor and low tail drawdown.
"""
...
class Alpha42TrendAlignedDualCompression(BaseHypothesis, BaseStrategy):
    strategy_id = "42_alpha"
    hypothesis_id = "42_alpha"
    name = "42_alpha — Trend-Aligned Dual Compression"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="MULTI_DAY_TREND_VOLATILITY_COMPRESSION",
            economic_rationale=(
                "Dual compression (Inside Day + NR4) within an established daily trend coils institutional inventory. "
                "The opening volume shock triggers rapid directional follow-through aligned with the primary trend."
            ),
            target_instruments=[],
            timeframe="15m",
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        bear_trend = ema20 < ema50

        # Pattern: Inside Day + NR4 on Day T-1
        ...
        dual_compression = is_id & is_nr4

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["dual_comp"] = pd.Series(dates, index=out.index).map(dual_compression).ffill().fillna(False)
        out["bull_trend"] = pd.Series(dates, index=out.index).map(bull_trend.shift(1)).ffill().fillna(False)
        out["bear_trend"] = pd.Series(dates, index=out.index).map(bear_trend.shift(1)).ffill().fillna(False)
        ...
        dual_flags = out["dual_comp"].values
        bull_flags = out["bull_trend"].values
        bear_flags = out["bear_trend"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if dual_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
