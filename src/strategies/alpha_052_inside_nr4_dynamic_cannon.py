"""
Ashva Quantitative Strategy: Inside NR4 Dynamic Volatility Cannon (Alpha 52)
Category: DUAL_COMPRESSION_DYNAMIC_EXPANSION
Market Mechanism: BREAKOUT

Hypothesis:
When an equity experiences an Inside Day that is simultaneously an NR4 (the narrowest range of the past 4 sessions),
short-term volatility reaches a coiled spring state. An opening 15m impulse on Day T with RVOL >= 1.15x and a decisive
directional candle body (>= 0.60) triggers explosive trend expansion with high win rate (>= 58%) and robust post-tax profit factor.
"""
...
class Alpha52InsideNR4DynamicCannon(BaseHypothesis, BaseStrategy):
    strategy_id = "52_alpha"
    hypothesis_id = "52_alpha"
    name = "52_alpha — Inside NR4 Dynamic Volatility Cannon"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="DUAL_COMPRESSION_DYNAMIC_EXPANSION",
            economic_rationale=(
                "Inside Day coinciding with a 4-day narrowest range (Inside-NR4) captures short-term volatility coiling. "
                "The subsequent opening volume catalyst triggers rapid trend expansion as market orders sweep the empty book."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="52_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # NR4 on Day T-1: Range(T-1) < Range(T-2), Range(T-3), Range(T-4)
        min_prev_3 = rng.shift(2).rolling(3, min_periods=3).min()
        is_nr4 = rng.shift(1) < min_prev_3

        # Inside Day on Day T-1: H(T-1) < H(T-2) AND L(T-1) > L(T-2)
        is_id = (h.shift(1) < h.shift(2)) & (l.shift(1) > l.shift(2))
        is_id_nr4 = is_id & is_nr4

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_id_nr4"] = pd.Series(dates, index=out.index).map(is_id_nr4).ffill().fillna(False)
        ...
        id_nr4_flags = out["is_id_nr4"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if id_nr4_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
