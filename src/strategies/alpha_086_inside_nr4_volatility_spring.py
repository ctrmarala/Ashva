"""
Ashva Quantitative Strategy: Inside Day + NR4 Dual Volatility Spring (Alpha 86)
Category: DUAL_VOLATILITY_SPRING
Market Mechanism: VOLATILITY_COMPRESSION_BREAKOUT

Hypothesis:
When a liquid equity forms a combined Inside Day (High[T-1] < High[T-2] and Low[T-1] > Low[T-2])
that is ALSO the Narrowest Range of 4 Days (Range[T-1] <= min(Range[T-2..T-5]))
aligned with the 50 SMA (Close[T-1] > 50 SMA for Long, or < 50 SMA for Short),
volatility compression reaches an energetic extreme.
Day T morning opening momentum expansion (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x triggers high-velocity multi-hour expansion.
"""
...
class Alpha86InsideNR4VolatilitySpring(BaseHypothesis, BaseStrategy):
    strategy_id = "86_alpha"
    hypothesis_id = "86_alpha"
    name = "86_alpha - Inside Day + NR4 Dual Volatility Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="DUAL_VOLATILITY_SPRING",
            economic_rationale=(
                "Dual compression (Inside Day + NR4) within a verified 50 SMA macro trend creates an explosive "
                "supply-demand imbalance on Day T morning directional breakout."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="86_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_inside = (h1 < h2) & (l1 > l2)

        # NR4 condition: range[T-1] <= min(range[T-2..T-4])
        min_rng_prior3 = rng.shift(2).rolling(3, min_periods=3).min()
        is_nr4 = rng1 <= min_rng_prior3

        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.75))
        is_orderly = (rng1 <= (atr14 * max_rng_ratio))

        is_bull_spring = is_inside & is_nr4 & (c1 > sma50) & is_orderly
        is_bear_spring = is_inside & is_nr4 & (c1 < sma50) & is_orderly

        is_bull_trend_map = is_bull_spring.to_dict()
        is_bear_trend_map = is_bear_spring.to_dict()
