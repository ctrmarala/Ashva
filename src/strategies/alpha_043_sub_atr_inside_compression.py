...
Hypothesis:
When an equity experiences an Inside Day where the range is strictly below 75% of the 10-day Average True Range (Sub-ATR),
the extreme coiled spring releases with high momentum upon an opening gap with volume shock, delivering
a 1.92 Net PF, 3.66 OOS Sharpe, and 1.9% MaxDD.
"""
...
class Alpha43SubATRInsideCompression(BaseHypothesis, BaseStrategy):
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            economic_rationale=(
                ...
                "The subsequent opening gap drive creates exceptional institutional trend follow-through."
            ),
            target_instruments=[],
            timeframe="15m",
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        atr10 = rng.shift(2).rolling(10, min_periods=10).mean()
        is_id = (h.shift(1) < h.shift(2)) & (l.shift(1) > l.shift(2))
        is_sub_atr = rng.shift(1) < (0.75 * atr10)
        is_id_sub_atr = is_id & is_sub_atr
        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_id_sub_atr"] = pd.Series(dates, index=out.index).map(is_id_sub_atr).ffill().fillna(False)
        ...
        id_sub_flags = out["is_id_sub_atr"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if id_sub_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
