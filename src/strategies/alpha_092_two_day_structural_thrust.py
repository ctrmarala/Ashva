"""
Ashva Quantitative Strategy: 2-Day Higher-High / Lower-Low Structural Thrust (Alpha 92)
Category: MULTI_DAY_STRUCTURAL_THRUST
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity exhibits a verified 2-day Wyckoff structural progression
(High[T-1] > High[T-2] and Low[T-1] > Low[T-2] for Long, or High[T-1] < High[T-2] and Low[T-1] < Low[T-2] for Short)
aligned with the 50 SMA (Close[T-1] > 50 SMA for Long, or < 50 SMA for Short) and orderly range (Range[T-1] <= 0.70x ATR),
institutional supply-absorption creates strong continuation on Day T.
A confirming morning directional breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x triggers rapid intraday trend expansion.
"""
...
class Alpha92TwoDayStructuralThrust(BaseHypothesis, BaseStrategy):
    strategy_id = "92_alpha"
    hypothesis_id = "92_alpha"
    name = "92_alpha - 2-Day Structural Trend Thrust"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="MULTI_DAY_STRUCTURAL_THRUST",
            economic_rationale=(
                "Two-day structural higher-highs and higher-lows with tight range (<= 0.70x ATR) and 50 SMA alignment "
                "demonstrates institutional accumulation ready for morning breakout expansion."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="92_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_2d_bull = (h1 > h2) & (l1 > l2) & (c1 > sma50)
        is_2d_bear = (h1 < h2) & (l1 < l2) & (c1 < sma50)
