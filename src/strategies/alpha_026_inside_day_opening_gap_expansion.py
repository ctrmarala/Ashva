"""
Ashva Quantitative Alpha Strategy — Alpha 26: Inside Day Opening Gap Expansion (IDE-15M)
Alpha ID: 26_alpha
Version: v1.0.0
Author: AshvaQuantLab

Economic Hypothesis:
When Day T-1 is an Inside Day (High_T1 < High_T2 and Low_T1 > Low_T2), the market is in equilibrium compression.
When Day T opens with an aligned directional gap (0.35%-1.80%) breaking the T-1 extreme on the 09:15 bar with RVOL >= 1.20x
and strong body (>= 55%), price expands directionally with tight structural risk (09:15 candle extreme).

Contract Specification:
- Mechanism: BREAKOUT / VOLATILITY_EXPANSION
- Timeframe: 15m
- Horizon: INTRADAY (15:15 IST hard square-off)
- Pure 1-bar impulse signal to prevent continuous-signal re-entry multiplier.
"""
...
class Alpha26InsideDayOpeningGapExpansion(BaseHypothesis, BaseStrategy):
    """
    Alpha 26: Inside Day Opening Gap Expansion (IDE-15M).
    Pure 1-bar impulse signal with candle-low/high structural stop and 1.5R target.
    """

    strategy_id = "26_alpha"
    hypothesis_id = "26_alpha"
    name = "26_alpha — Inside Day Opening Gap Expansion"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        BaseStrategy.__init__(self, strategy_id="26_alpha", parameters=merged)
        self._current_pos: Dict[str, float] = {}

    def get_parameter_grid(self) -> Dict[str, List[Any]]:
        return {
            "min_gap": [0.0040, 0.0050],
            "target_rr": [1.50, 1.75],
        }
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        max_gap = float(self.parameters.get("max_gap", 0.0180))
