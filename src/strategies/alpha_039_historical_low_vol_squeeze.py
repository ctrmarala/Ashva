"""
Ashva Quantitative Strategy: Historical Low-Volatility Squeeze with Volume Shock (Alpha 39)
Category: VOLATILITY_SQUEEZE
Market Mechanism: BREAKOUT

Hypothesis:
When an equity's 10-day historical Parkinson volatility reaches its lowest level in 30 days (Extreme Volatility Squeeze)
AND Day T-1 forms an Inside Day, the market is in an unsustainable coiled equilibrium.
An opening gap on Day T with institutional Relative Volume (RVOL >= 1.50x) triggers high-velocity explosive expansion
generating high win rate and robust post-tax profit factor.
"""
...
class Alpha39HistoricalLowVolSqueeze(BaseHypothesis, BaseStrategy):
    strategy_id = "39_alpha"
    hypothesis_id = "39_alpha"
    name = "39_alpha — Historical Low-Volatility Squeeze with Volume Shock"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="VOLATILITY_SQUEEZE",
            economic_rationale=(
                "When 10-day volatility reaches multi-week lows combined with an Inside Day compression, "
                "the resulting coiled spring explodes into persistent intraday directional trends."
            ),
            target_instruments=[],
            timeframe="15m",
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Parkinson Historical Volatility proxy: log(H/L)^2 / (4 * ln(2))
        log_hl = np.log(h / np.maximum(1e-6, l))
        pv = np.sqrt(log_hl**2 / (4 * np.log(2)))
        hv10 = pv.rolling(10, min_periods=10).mean()
        min_hv30 = hv10.rolling(20, min_periods=10).min()
        
        # Volatility squeeze: HV10 at or near 20-day lowest AND Inside Day
        is_vol_squeeze = (hv10.shift(1) <= min_hv30.shift(1) * 1.10) & (h.shift(1) < h.shift(2)) & (l.shift(1) > l.shift(2))
        
        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_vol_squeeze"] = pd.Series(dates, index=out.index).map(is_vol_squeeze).ffill().fillna(False)
        ...
        squeeze_flags = out["is_vol_squeeze"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if squeeze_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
