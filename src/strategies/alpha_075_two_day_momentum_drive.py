"""
Ashva Quantitative Strategy: 2-Day Compound Momentum Acceleration Drive (Alpha 75)
Category: MULTI_DAY_MOMENTUM_DRIVE
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity exhibits 2 consecutive days of expanding directional closes (Close[T-1] >= Close[T-2] * 1.008
and Close[T-2] > Close[T-3] for Long, or Close[T-1] <= Close[T-2] * 0.992 and Close[T-2] < Close[T-3] for Short)
aligned with the 50 SMA (Close[T-1] > 50 SMA) and controlled volatility (Range[T-1] <= 0.85x ATR),
Day T morning opening momentum drive (09:15-09:30 IST) with gap >= 0.35%, solid body >= 65%, and RVOL >= 1.25x
captures high-velocity institutional trend continuation independent of inside-day compression.
"""
...
class Alpha75TwoDayMomentumDrive(BaseHypothesis, BaseStrategy):
    strategy_id = "75_alpha"
    hypothesis_id = "75_alpha"
    name = "75_alpha — 2-Day Compound Momentum Acceleration Drive"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="MULTI_DAY_MOMENTUM_DRIVE",
            economic_rationale=(
                "Two consecutive days of strong percentage accumulation creates institutional follow-through "
                "on Day T morning directional breakouts, independent of inside-day regimes."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="75_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 2-day compound acceleration strictly shifted by 1 day (zero lookahead)
        c1 = c.shift(1)
        c2 = c.shift(2)
        c3 = c.shift(3)
        h1 = h.shift(1)
        ...
        is_2d_bull = (c1 >= c2 * 1.008) & (c2 > c3) & (c1 > sma50)
        is_2d_bear = (c1 <= c2 * 0.992) & (c2 < c3) & (c1 < sma50)
        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.85))
        is_orderly = (rng1 <= (atr14 * max_rng_ratio))

        is_bull_trend_map = (is_2d_bull & is_orderly).to_dict()
        is_bear_trend_map = (is_2d_bear & is_orderly).to_dict()
