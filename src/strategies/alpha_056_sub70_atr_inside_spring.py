"""
Ashva Quantitative Strategy: Sub-0.70 ATR Inside Compression Spring (Alpha 56)
Category: MULTI_TIMEFRAME_VOLATILITY_COMPRESSION
Market Mechanism: BREAKOUT

Hypothesis:
When an equity experiences an Inside Day on Day T-1 where the daily trading range compresses below 70% of its 10-day ATR,
volatility potential energy accumulates at a steep rate.
An opening 15m directional breakout impulse on Day T with RVOL >= 1.20x and solid candle body (>= 0.60)
triggers high-conviction trend expansion with superior institutional win rate and profit factor.
"""
...
class Alpha56Sub70ATRInsideSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "56_alpha"
    hypothesis_id = "56_alpha"
    name = "56_alpha — Sub-0.70 ATR Inside Compression Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="MULTI_TIMEFRAME_VOLATILITY_COMPRESSION",
            economic_rationale=(
                "Inside Day with sub-0.70x ATR range represents severe volatility suppression. "
                "The subsequent opening auction impulse provides explosive directional trend follow-through."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="56_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_sub70 = rng.shift(1) < (0.70 * atr10)
        is_spring = is_id & is_sub70

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_spring"] = pd.Series(dates, index=out.index).map(is_spring).ffill().fillna(False)
        ...
        spring_flags = out["is_spring"].values
