"""
Ashva Quantitative Strategy: 7-Day Donchian Trend Channel Expansion (Alpha 85)
Category: MULTI_DAY_DONCHIAN_EXPANSION
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity breaks out of its 7-day Donchian Channel (Close[T-1] >= max(High[T-2..T-8]) for Long,
or Close[T-1] <= min(Low[T-2..T-8]) for Short) with orderly volatility (Range[T-1] <= 0.75x ATR)
and moving average alignment (Close[T-1] > EMA20 > SMA50), institutional momentum carries into Day T.
...
class Alpha85MultiDayDonchianTrendExpansion(BaseHypothesis, BaseStrategy):
    strategy_id = "85_alpha"
    hypothesis_id = "85_alpha"
    name = "85_alpha - 7-Day Donchian Trend Channel Expansion"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="MULTI_DAY_DONCHIAN_EXPANSION",
            economic_rationale=(
                "7-day Donchian Channel breakout on controlled daily range (<= 0.75x ATR) and EMA20 > SMA50 alignment "
                "captures early institutional trend expansion without short-term exhaustion."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="85_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        ema20 = c.shift(1).ewm(span=20, min_periods=10).mean()
        ...
        # 7-day rolling highest high and lowest low (T-2 to T-8)
        donchian_h7 = h.shift(2).rolling(7, min_periods=5).max()
        donchian_l7 = l.shift(2).rolling(7, min_periods=5).min()

        is_donchian_bull = (c1 >= donchian_h7) & (c1 > ema20) & (ema20 > sma50)
        is_donchian_bear = (c1 <= donchian_l7) & (c1 < ema20) & (ema20 < sma50)
        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.75))
        is_orderly = (rng1 <= (atr14 * max_rng_ratio))

        is_bull_trend_map = (is_donchian_bull & is_orderly).to_dict()
        is_bear_trend_map = (is_donchian_bear & is_orderly).to_dict()
