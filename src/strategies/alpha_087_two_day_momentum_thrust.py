"""
Ashva Quantitative Strategy: 2-Day Trend Momentum Thrust (Alpha 87)
Category: MULTI_DAY_MOMENTUM_THRUST
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity exhibits 2 consecutive strong directional closes breaking the prior swing boundary
(Close[T-1] > Close[T-2] > High[T-3] for Long, or Close[T-1] < Close[T-2] < Low[T-3] for Short)
aligned with the 50 SMA (Close[T-1] > 50 SMA for Long, or < 50 SMA for Short) and orderly range (Range[T-1] <= 0.75x ATR),
institutional buying thrust continues into Day T.
A confirming morning directional breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x triggers rapid continuation that operates orthogonally to inside-day regimes.
"""
...
class Alpha87TwoDayMomentumThrust(BaseHypothesis, BaseStrategy):
    strategy_id = "87_alpha"
    hypothesis_id = "87_alpha"
    name = "87_alpha - 2-Day Trend Momentum Thrust"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="MULTI_DAY_MOMENTUM_THRUST",
            economic_rationale=(
                "Two-day consecutive directional close expansion breaking prior swing high/low on controlled range "
                "creates high-velocity institutional continuation on Day T morning gap breakouts."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="87_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_2d_bull = (c1 > c2) & (c2 > h3) & (c1 > sma50)
        is_2d_bear = (c1 < c2) & (c2 < l3) & (c1 < sma50)
