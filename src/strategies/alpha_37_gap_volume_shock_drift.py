"""
Ashva Quantitative Strategy: Overnight Gap Volume Shock Momentum Drift (Alpha 37)
Captures persistent institutional order-flow continuation following high-volume overnight gap imbalances.

Hypothesis:
When a liquid equity opens with a moderate overnight gap (0.30% to 1.50%) and the opening 15-minute bar
exhibits extreme volume shock (RVOL >= 1.75x) with strong directional body (>= 60%), aggressive institutional
inventory rebalancing drives persistent intraday drift into a 1.50R target.
"""
...
class Alpha37GapVolumeShockDrift(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_37_GAP_VOLUME_SHOCK_DRIFT",
            name="Alpha_37_Gap_Volume_Shock_Drift",
            category="ORDER_FLOW_IMBALANCE",
            economic_rationale=(
                "Extreme volume shock (RVOL >= 1.75x) on opening gap bars indicates heavy institutional block participation, "
                "yielding strong directional drift with low adverse excursion."
            ),
            target_instruments=[
        ...
        for i in range(n):
            ...
            if bar_time >= t_1515:
                if curr_state != 0.0:
                    curr_state = 0.0
                    signals[i] = 0.0
                    rationales[i] = "Alpha 37 EXIT: 15:15 EOD Square-Off"
                continue
                ...
                if min_gap <= abs_gap <= max_gap:
                    ...
                    if bar_range > 0 and (body_size / bar_range) >= min_body and rvol >= min_rvol:
                        ...
                        if gap_pct > 0 and closes[i] > opens[i]:
                            ...
                            curr_sl = sl
                            curr_tp = tp
                            curr_rationale = f"Alpha 37 LONG: Gap=+{gap_pct*100:.2f}% | RVOL={rvol:.2f}x | Body={body_size/bar_range*100:.0f}%"
                            signals[i] = 1.0
                            stop_loss[i] = curr_sl
                            take_profit[i] = curr_tp
                            rationales[i] = curr_rationale
                        ...
                        # Bearish Gap Continuation (SHORT)
                            ...
                            curr_sl = sl
                            curr_tp = tp
                            curr_rationale = f"Alpha 37 SHORT: Gap={gap_pct*100:.2f}% | RVOL={rvol:.2f}x | Body={body_size/bar_range*100:.0f}%"
                            signals[i] = -1.0
                            stop_loss[i] = curr_sl
                            take_profit[i] = curr_tp
                            rationales[i] = curr_rationale
