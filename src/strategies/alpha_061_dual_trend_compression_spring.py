"""
Ashva Quantitative Strategy: Dual Trend 50 SMA / 20 EMA Compression Spring (Alpha 61)
Category: TREND_ALIGNED_COMPRESSION_SPRING
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When an equity is in confirmed macro-trend alignment (Close > 50 SMA and Close > 20 EMA for Long, 
or Close < 50 SMA and Close < 20 EMA for Short) and exhibits daily volatility compression 
(Daily Range <= 0.80x 14-day ATR), institutional inventory rests before a powerful trend surge.
On Day T, a 15-minute morning opening directional breakout beyond Day T-1 extreme triggers
high-expectancy momentum expansion that decorrelates from inside-day regimes.
"""
...
class Alpha61DualTrendCompressionSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "61_alpha"
    hypothesis_id = "61_alpha"
    name = "61_alpha — Dual Trend 50 SMA / 20 EMA Compression Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="TREND_ALIGNED_COMPRESSION_SPRING",
            economic_rationale=(
                "Macro trend alignment with 50 SMA and 20 EMA combined with sub-0.80x ATR daily compression "
                "creates high-velocity trend continuation breakouts on Day T morning drives."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="61_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        ema20 = c.shift(2).ewm(span=20, adjust=False).mean()

        is_compressed = rng.shift(1) <= (0.80 * atr14)
        bull_trend_spring = (c.shift(1) > sma50) & (c.shift(1) > ema20) & is_compressed
        bear_trend_spring = (c.shift(1) < sma50) & (c.shift(1) < ema20) & is_compressed
