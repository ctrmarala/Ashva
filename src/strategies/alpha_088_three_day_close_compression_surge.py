"""
Ashva Quantitative Strategy: 3-Day Close Accumulation + Range Compression Surge (Alpha 88)
Category: CONSECUTIVE_ACCUMULATION_COMPRESSION
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity exhibits 3 consecutive directional closes (Close[T-1] > Close[T-2] > Close[T-3] for Long,
or Close[T-1] < Close[T-2] < Close[T-3] for Short) with tight volatility compression on Day T-1 (Range[T-1] <= 0.65x ATR)
and moving average alignment (Close[T-1] > EMA20 > SMA50 for Long, or < EMA20 < SMA50 for Short),
a potent volatility spring is coiled.
A confirming morning directional breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x triggers high-velocity continuation that operates orthogonally to inside-day regimes.
"""
...
class Alpha88ThreeDayCloseCompressionSurge(BaseHypothesis, BaseStrategy):
    strategy_id = "88_alpha"
    hypothesis_id = "88_alpha"
    name = "88_alpha - 3-Day Close Accumulation + Range Compression Surge"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="CONSECUTIVE_ACCUMULATION_COMPRESSION",
            economic_rationale=(
                "Consecutive 3-day directional close accumulation combined with extreme Day T-1 range compression "
                "(<= 0.65x ATR) and EMA20 > SMA50 alignment creates a high-conviction breakout spring on Day T morning gap-ups."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="88_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_3d_bull = (c1 > c2) & (c2 > c3) & (c1 > ema20) & (ema20 > sma50)
        is_3d_bear = (c1 < c2) & (c2 < c3) & (c1 < ema20) & (ema20 < sma50)
