"""
Ashva Quantitative Strategy: Ultra-Deep Sub-0.60 ATR Inside Spring (Alpha 54)
Category: ULTRA_DEEP_VOLATILITY_COMPRESSION
Market Mechanism: BREAKOUT

Hypothesis:
When an equity experiences an Inside Day where the range compresses below 60% of its 10-day ATR (Ultra-Deep Compression),
the coiled potential energy reaches maximum density. An opening 15m breakout drive on Day T with RVOL >= 1.20x
and decisive directional candle body (>= 0.60) produces explosive trend expansion with superior win rate and high Sharpe ratio.
"""
...
class Alpha54UltraDeepInsideSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "54_alpha"
    hypothesis_id = "54_alpha"
    name = "54_alpha — Ultra-Deep Sub-0.60 ATR Inside Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="ULTRA_DEEP_VOLATILITY_COMPRESSION",
            economic_rationale=(
                "Inside Day with sub-0.60x ATR range represents maximum coiling of institutional inventory. "
        ...
        BaseStrategy.__init__(self, strategy_id="54_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_ultra_sub_atr = rng.shift(1) < (0.60 * atr10)
        is_ultra_spring = is_id & is_ultra_sub_atr

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_ultra_spring"] = pd.Series(dates, index=out.index).map(is_ultra_spring).ffill().fillna(False)
        ...
        spring_flags = out["is_ultra_spring"].values
