"""
Alpha 80: NR7 Volatility Contraction Breakout (ALPHA_80_NR7_VOLATILITY_EXPANSION)
Identifies 7-day range contraction (NR7, narrowest range in 7 days) that breaks directionally
beyond the previous day high/low on morning volume surge with asymmetric 2.0R target.
"""
...
class Alpha80NR7VolatilityExpansion(BaseHypothesis):
    """
    Alpha 80: NR7 Volatility Contraction Breakout
    Combines 7-day range contraction with opening expansion and RVOL >= 1.25x.
    """
    ...
    def __init__(
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_80_NR7_VOLATILITY_EXPANSION",
            name="Alpha_80_NR7_Volatility_Expansion",
            category="VOLATILITY_EXPANSION",
            economic_rationale=(
                "NR7 7-day multi-day volatility compression followed by a directional opening breakout "
                "with volume creates massive trending impulse moves toward 2.0R targets."
            ),
            target_instruments=[
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out["is_nr7"] = pd.Series(dates, index=out.index).map(is_nr7_series).ffill().fillna(False)
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if nr7_flags[i] and abs_gap >= min_gap and rvol >= min_rvol:
                    if gap_pct > 0 and closes[i] > prev_highs[i] and closes[i] > opens[i]:
                        ...
                        curr_rationale = f"Alpha80 LONG: NR7 Expansion, Gap +{gap_pct*100:.2f}%, RVOL {rvol:.1f}x, RR {target_rr}"
                        ...
                        tp = closes[i] - (target_rr * risk)
                        curr_sl, curr_tp = sl, tp
                        curr_rationale = f"Alpha80 SHORT: NR7 Expansion, Gap {gap_pct*100:.2f}%, RVOL {rvol:.1f}x, RR {target_rr}"
                        signals[i], stop_loss[i], take_profit[i] = -1.0, curr_sl, curr_tp
                        rationales[i] = curr_rationale
                        traded_today = True
