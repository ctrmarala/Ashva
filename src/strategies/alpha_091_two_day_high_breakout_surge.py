"""
Ashva Quantitative Strategy: 2-Day Prior-High Close Breakout Surge (Alpha 91)
Category: MULTI_DAY_STRUCTURAL_BREAKOUT
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity closes above the high of Day T-2 (Close[T-1] > High[T-2] for Long, or Close[T-1] < Low[T-2] for Short)
with orderly volatility on Day T-1 (Range[T-1] <= 0.70x ATR) and macro trend alignment (Close[T-1] > 50 SMA for Long, or < 50 SMA for Short),
a structural swing breakout is established with low friction.
...
class Alpha91TwoDayHighBreakoutSurge(BaseHypothesis, BaseStrategy):
    strategy_id = "91_alpha"
    hypothesis_id = "91_alpha"
    name = "91_alpha - 2-Day Prior-High Close Breakout Surge"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="MULTI_DAY_STRUCTURAL_BREAKOUT",
            economic_rationale=(
                "Closing above Day T-2 high with controlled Day T-1 range (<= 0.70x ATR) confirms structural momentum without exhaustion. "
                "Day T morning gap continuation provides high-expectancy trend expansion."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="91_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_breakout_bull = (c1 > h2) & (c1 > sma50)
        is_breakout_bear = (c1 < l2) & (c1 < sma50)
        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.70))
        is_orderly = (rng1 <= (atr14 * max_rng_ratio))

        is_bull_trend_map = (is_breakout_bull & is_orderly).to_dict()
        is_bear_trend_map = (is_breakout_bear & is_orderly).to_dict()
