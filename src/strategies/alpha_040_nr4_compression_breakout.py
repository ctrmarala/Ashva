"""
Ashva Quantitative Strategy: NR4 Multi-Session Volatility Compression Breakout (Alpha 40)
Category: VOLATILITY_EXPANSION
Market Mechanism: BREAKOUT

Hypothesis:
When an equity experiences a Narrowest Range in 4 Days (NR4) on Day T-1, market participants have reached a deep
volatility equilibrium. An opening gap or 09:15-09:30 impulse breaking Day T-1 extreme with morning
Relative Volume (RVOL >= 1.30x) unleashes high-velocity expansion, capturing large R-multiple trend days.
"""
...
class Alpha40NR4CompressionBreakout(BaseHypothesis, BaseStrategy):
    strategy_id = "40_alpha"
    hypothesis_id = "40_alpha"
    name = "40_alpha — NR4 Multi-Session Volatility Compression Breakout"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="VOLATILITY_EXPANSION",
            economic_rationale=(
                "NR4 represents severe multi-session volatility contraction. The subsequent morning breakout "
                "with volume shock delivers high-probability institutional trend follow-through."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="40_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # NR4 condition on Day T-1: Range(T-1) < min(Range(T-2), Range(T-3), Range(T-4))
        is_nr4 = daily_range.shift(1) < daily_range.shift(2).rolling(3, min_periods=3).min()
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if nr4_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
