"""
Ashva Quantitative Strategy: 3-Day Momentum Continuation Surge 2.0R (Alpha 86)
Captures powerful multi-day momentum continuation when an established 3-day directional trend opens with an aligned surge.

Hypothesis:
When an equity exhibits 3 consecutive days of higher highs and higher lows (established institutional accumulation),
an aligned opening gap (>= 0.20%) accompanied by a high-volume opening expansion bar (RVOL >= 1.35x) signals intense
continuation order flow that easily reaches a 2.0R target before 15:15 IST.
"""
...
class Alpha86ThreeDayTrendSurge2R(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_86_THREE_DAY_TREND_SURGE_2R",
            name="Alpha_86_Three_Day_Trend_Surge_2R",
            category="MOMENTUM",
            economic_rationale=(
                "Three consecutive sessions of directional accumulation followed by an aligned opening drive "
            ...
            if bar_time == t_0915:
                ...
                if is_up_flags[i] and gap >= min_gap and closes[i] > opens[i] and (body / max(1e-5, bar_range)) >= min_body and rvol >= min_rvol:
                    ...
                    if 0.0025 * closes[i] <= risk <= 0.0120 * closes[i]:
                        ...
                        stop_loss[i] = sl
                        take_profit[i] = tp
                        rationales[i] = f"Alpha 86 LONG: 3D Trend Surge Gap={gap*100:.2f}% RVOL={rvol:.2f}x"
                        traded_today = True
                ...
                # Bearish 3-Day Trend Surge (SHORT)
                    ...
                    if 0.0025 * closes[i] <= risk <= 0.0120 * closes[i]:
                        ...
                        stop_loss[i] = sl
                        take_profit[i] = tp
                        rationales[i] = f"Alpha 86 SHORT: 3D Trend Surge Gap={gap*100:.2f}% RVOL={rvol:.2f}x"
                        traded_today = True
