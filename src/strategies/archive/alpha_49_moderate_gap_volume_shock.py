"""
Ashva Quantitative Strategy: Moderate Gap Volume Shock Drift (Alpha 49)
Captures persistent directional momentum following high-volume moderate overnight gaps (0.35% to 1.20%).

Hypothesis:
When an equity opens with a moderate overnight gap (0.35% to 1.20%) and Bar 1 (09:15-09:30 IST) exhibits
heavy volume shock (RVOL >= 2.00x) with solid directional candle body (>= 60%), institutional block rebalancing
drives clean directional continuation toward a 1.50R target, squared off by 15:15 IST.
"""
...
class Alpha49ModerateGapVolumeShock(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_49_MODERATE_GAP_VOLUME_SHOCK",
            name="Alpha_49_Moderate_Gap_Volume_Shock",
            category="ORDER_FLOW_IMBALANCE",
            economic_rationale=(
                "Moderate overnight gaps (0.35% to 1.20%) accompanied by 2.0x volume shock avoid retail gap-trap "
                "exhaustion while capturing strong institutional order-flow continuation."
            ),
            target_instruments=[
        ...
        for i in range(n):
            ...
            if bar_time >= t_1515:
                if curr_state != 0.0:
                    curr_state = 0.0
                    signals[i] = 0.0
                    rationales[i] = "Alpha 49 EXIT: 15:15 EOD Square-Off"
                continue
                ...
                if min_gap <= abs_gap <= max_gap:
                    ...
                    if bar_range > 0 and (body_size / bar_range) >= min_body and rvol >= min_rvol:
                        ...
                        if gap_pct > 0 and closes[i] > opens[i]:
                            ...
                            curr_sl = sl
                            curr_tp = tp
                            curr_rationale = f"Alpha 49 LONG: Gap=+{gap_pct*100:.2f}% | RVOL={rvol:.2f}x | Body={body_size/bar_range*100:.0f}%"
                            signals[i] = 1.0
                            stop_loss[i] = curr_sl
                            take_profit[i] = curr_tp
                            rationales[i] = curr_rationale
                        ...
                        # Bearish Gap Continuation (SHORT)
                            ...
                            curr_sl = sl
                            curr_tp = tp
                            curr_rationale = f"Alpha 49 SHORT: Gap={gap_pct*100:.2f}% | RVOL={rvol:.2f}x | Body={body_size/bar_range*100:.0f}%"
                            signals[i] = -1.0
                            stop_loss[i] = curr_sl
                            take_profit[i] = curr_tp
                            rationales[i] = curr_rationale
