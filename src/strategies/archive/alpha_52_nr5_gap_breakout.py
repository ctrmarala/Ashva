"""
Ashva Quantitative Strategy: NR5 Volatility Compression Gap Breakout (Alpha 52)
Captures directional intraday trend expansion when a 5-day narrow range (NR5) is resolved by an aligned opening gap.

Hypothesis:
When Day T-1 range is the narrowest of the prior 5 sessions (NR5 condition), multi-day range compression
reaches critical tension. On Day T, an overnight gap (>= 0.30%) breaking Day T-1 high/low with heavy volume shock
(RVOL >= 1.75x) triggers institutional continuation toward a 1.75R target, squared off by 15:15 IST.
"""
...
class Alpha52NR5GapBreakout(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_52_NR5_GAP_BREAKOUT",
            name="Alpha_52_NR5_Gap_Breakout",
            category="VOLATILITY_CONTRACTION_EXPANSION",
            economic_rationale=(
                "NR5 represents a 5-session market balance. An aligned opening gap breaking prior day high/low "
                "with 1.75x volume shock triggers strong multi-session delta expansion."
            ),
            target_instruments=[
        ...
        for i in range(n):
            ...
            if bar_time >= t_1515:
                if curr_state != 0.0:
                    curr_state = 0.0
                    signals[i] = 0.0
                    rationales[i] = "Alpha 52 EXIT: 15:15 EOD Square-Off"
                continue
            ...
            if bar_time == t_0915:
                ...
                if bar_range > 0 and (body_size / bar_range) >= min_body and rvol >= min_rvol:
                    ...
                    if gap >= min_gap and closes[i] > prev_highs[i] and closes[i] > opens[i]:
                        ...
                        curr_sl = sl
                        curr_tp = tp
                        curr_rationale = f"Alpha 52 LONG: NR5 Gap Breakout Close={closes[i]:.1f} > PDH={prev_highs[i]:.1f} | RVOL={rvol:.2f}x"
                        signals[i] = 1.0
                        stop_loss[i] = curr_sl
                        take_profit[i] = curr_tp
                        rationales[i] = curr_rationale
                    ...
                    # Bearish NR5 Gap Breakout (SHORT)
                        ...
                        curr_sl = sl
                        curr_tp = tp
                        curr_rationale = f"Alpha 52 SHORT: NR5 Gap Breakout Close={closes[i]:.1f} < PDL={prev_lows[i]:.1f} | RVOL={rvol:.2f}x"
                        signals[i] = -1.0
                        stop_loss[i] = curr_sl
                        take_profit[i] = curr_tp
                        rationales[i] = curr_rationale
