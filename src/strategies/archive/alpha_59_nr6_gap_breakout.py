"""
Ashva Quantitative Strategy: NR6 Volatility Compression Gap Breakout (Alpha 59)
Captures directional intraday trend expansion when a 6-day narrow range (NR6) is resolved by an aligned opening gap.

Hypothesis:
When Day T-1 range is the narrowest of the prior 6 sessions (NR6 condition), multi-day equilibrium reaches
exceptional tightness. On Day T, an overnight gap (>= 0.30%) breaking Day T-1 high/low on heavy volume (RVOL >= 1.75x)
triggers explosive institutional delta expansion toward a 1.75R target, squared off by 15:15 IST.
"""
...
class Alpha59NR6GapBreakout(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_59_NR6_GAP_BREAKOUT",
            name="Alpha_59_NR6_Gap_Breakout",
            category="VOLATILITY_CONTRACTION_EXPANSION",
            economic_rationale=(
                "NR6 represents 6 sessions of continuous range compression. An aligned opening gap with 1.75x volume "
                "triggers massive stop-loss cascades and programmatic directional trend expansion."
            ),
            target_instruments=[
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # NR6 calculation from completed prior sessions (Shifted 1 session)
        is_nr6 = pd.Series(False, index=daily_summary.index)
        for i in range(6, len(daily_summary)):
            t_minus_1_range = daily_range.iloc[i - 1]
            prior_5_ranges = daily_range.iloc[i - 6:i - 1]
            if t_minus_1_range < prior_5_ranges.min():
                is_nr6.iloc[i] = True
        ...
        out["is_nr6_day"] = pd.Series(dates, index=out.index).map(is_nr6).ffill().fillna(False)
        ...
        nr6_flags = out["is_nr6_day"].values
        ...
        for i in range(n):
            ...
            if bar_time >= t_1515:
                if curr_state != 0.0:
                    curr_state = 0.0
                    signals[i] = 0.0
                    rationales[i] = "Alpha 59 EXIT: 15:15 EOD Square-Off"
                continue
            ...
            if traded_today or not nr6_flags[i] or pd.isna(prev_closes[i]) or prev_closes[i] <= 0:
                continue
            ...
            if bar_time == t_0915:
                ...
                if bar_range > 0 and (body_size / bar_range) >= min_body and rvol >= min_rvol:
                    ...
                    if gap >= min_gap and closes[i] > prev_highs[i] and closes[i] > opens[i]:
                        ...
                        curr_sl = sl
                        curr_tp = tp
                        curr_rationale = f"Alpha 59 LONG: NR6 Gap Breakout Close={closes[i]:.1f} > PDH={prev_highs[i]:.1f} | RVOL={rvol:.2f}x"
                        signals[i] = 1.0
                        stop_loss[i] = curr_sl
                        take_profit[i] = curr_tp
                        rationales[i] = curr_rationale
                    ...
                    # Bearish NR6 Gap Breakout (SHORT)
                        ...
                        curr_sl = sl
                        curr_tp = tp
                        curr_rationale = f"Alpha 59 SHORT: NR6 Gap Breakout Close={closes[i]:.1f} < PDL={prev_lows[i]:.1f} | RVOL={rvol:.2f}x"
                        signals[i] = -1.0
                        stop_loss[i] = curr_sl
                        take_profit[i] = curr_tp
                        rationales[i] = curr_rationale
