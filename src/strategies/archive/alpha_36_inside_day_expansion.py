"""
Ashva Quantitative Strategy: Inside Day Opening Range Expansion (Alpha 36)
Captures directional intraday trend expansion following a 2-day inside compression (Inside Day).

Hypothesis:
When an equity forms an Inside Day (Day T-1 High < Day T-2 High and Day T-1 Low > Day T-2 Low),
two-session volatility compression stores structural kinetic energy. On the subsequent session,
an opening range breakout (09:30-10:30 IST) with elevated volume (RVOL >= 1.25x) triggers institutional
momentum follow-through toward a 1.50R target, squared off by 15:15 IST.
"""
...
class Alpha36InsideDayExpansion(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_36_INSIDE_DAY_EXPANSION",
            name="Alpha_36_Inside_Day_Expansion",
            category="VOLATILITY_CONTRACTION_EXPANSION",
            economic_rationale=(
                "Following an Inside Day equilibrium, an opening range breakout with volume traps prior two-session "
                "participants and triggers institutional momentum continuation."
            ),
            target_instruments=[
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        prev_h = daily_summary["day_high"].shift(1)
        prev_l = daily_summary["day_low"].shift(1)
        prev_prev_h = daily_summary["day_high"].shift(2)
        prev_prev_l = daily_summary["day_low"].shift(2)

        # Inside Day: Day T-1 is inside Day T-2 strictly from completed sessions
        is_inside = (prev_h < prev_prev_h) & (prev_l > prev_prev_l)
        ...
        for i in range(n):
            ...
            if bar_time >= t_1515:
                if curr_state != 0.0:
                    curr_state = 0.0
                    signals[i] = 0.0
                    rationales[i] = "Alpha 36 EXIT: 15:15 EOD Square-Off"
                continue
            ...
            c_tod = tod_vols[i]
            c_atr = daily_atrs[i]
            is_inside_today = inside_flags[i]
            ...
            if (not or_established) or (not is_inside_today) or traded_today or pd.isna(c_atr) or c_atr <= 0:
                continue
            ...
            if (hour == 9 and minute >= 30) or (hour == 10 and minute <= 30):
                ...
                if (c_close > or_high) and (c_close > c_open) and (rvol >= min_rvol):
                    ...
                    curr_rationale = f"Alpha 36 LONG: Inside Day Expansion Close={c_close:.1f} > OR_H={or_high:.1f} | RVOL={rvol:.2f}x"
                    signals[i] = 1.0
                    stop_loss[i] = curr_sl
                    take_profit[i] = curr_tp
                    rationales[i] = curr_rationale
                ...
                # Short Breakout
                    ...
                    curr_sl = c_close + stop_dist
                    curr_tp = c_close - (target_rr * stop_dist)
                    curr_rationale = f"Alpha 36 SHORT: Inside Day Expansion Close={c_close:.1f} < OR_L={or_low:.1f} | RVOL={rvol:.2f}x"
                    signals[i] = -1.0
                    stop_loss[i] = curr_sl
                    take_profit[i] = curr_tp
                    rationales[i] = curr_rationale
