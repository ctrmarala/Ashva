"""
Ashva Quantitative Strategy: NR3 Moderate Gap Volume Shock (Alpha 57)
Captures directional intraday trend expansion when a 3-day narrow range (NR3) is resolved by a moderate opening gap.

Hypothesis:
When Day T-1 range is the narrowest of the prior 3 sessions (NR3 condition) and Day T opens with a moderate gap
(0.35% to 1.20%) breaking Day T-1 range on 2.0x volume shock, the release of short-term compression produces
clean directional expansion toward a 1.50R target, squared off by 15:15 IST.
"""
...
class Alpha57NR3ModerateGapShock(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_57_NR3_MODERATE_GAP_SHOCK",
            name="Alpha_57_NR3_Moderate_Gap_Shock",
            category="VOLATILITY_CONTRACTION_EXPANSION",
            economic_rationale=(
                "NR3 range contraction combined with a high-conviction 2.0x volume shock on a moderate opening gap "
                "triggers strong institutional momentum that clears Indian friction hurdles."
            ),
            target_instruments=[
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # NR3 calculation from completed prior sessions (Shifted 1 session)
        is_nr3 = pd.Series(False, index=daily_summary.index)
        for i in range(3, len(daily_summary)):
            t_minus_1_range = daily_range.iloc[i - 1]
            prior_2_ranges = daily_range.iloc[i - 3:i - 1]
            if t_minus_1_range < prior_2_ranges.min():
                is_nr3.iloc[i] = True
        ...
        for i in range(n):
            ...
            if bar_time >= t_1515:
                if curr_state != 0.0:
                    curr_state = 0.0
                    signals[i] = 0.0
                    rationales[i] = "Alpha 57 EXIT: 15:15 EOD Square-Off"
                continue
            ...
            if bar_time == t_0915:
                ...
                if min_gap <= abs_gap <= max_gap:
                    ...
                    if bar_range > 0 and (body_size / bar_range) >= min_body and rvol >= min_rvol:
                        # Bullish NR3 Moderate Gap Breakout (LONG)
                        if gap_pct > 0 and closes[i] > prev_highs[i] and closes[i] > opens[i]:
                            ...
                            curr_sl = sl
                            curr_tp = tp
                            curr_rationale = f"Alpha 57 LONG: NR3 Moderate Gap Close={closes[i]:.1f} > PDH={prev_highs[i]:.1f} | RVOL={rvol:.2f}x"
                            signals[i] = 1.0
                            stop_loss[i] = curr_sl
                            take_profit[i] = curr_tp
                            rationales[i] = curr_rationale
                        ...
                        # Bearish NR3 Moderate Gap Breakout (SHORT)
                            ...
                            curr_sl = sl
                            curr_tp = tp
                            curr_rationale = f"Alpha 57 SHORT: NR3 Moderate Gap Close={closes[i]:.1f} < PDL={prev_lows[i]:.1f} | RVOL={rvol:.2f}x"
                            signals[i] = -1.0
                            stop_loss[i] = curr_sl
                            take_profit[i] = curr_tp
                            rationales[i] = curr_rationale
