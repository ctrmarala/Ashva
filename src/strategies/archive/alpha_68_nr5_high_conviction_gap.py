"""
Ashva Quantitative Strategy: NR5 High-Conviction Opening Gap Drift (Alpha 68)
Captures directional intraday trend expansion when a 5-day narrow range (NR5) is resolved by an opening gap on 2.25x volume.

Hypothesis:
When Day T-1 range is the narrowest of the prior 5 sessions (NR5 condition) and Day T opens with an aligned gap (>= 0.30%)
breaking Day T-1 range on extreme volume shock (RVOL >= 2.25x), multi-day potential energy releases with high
directional conviction toward a 1.75R target, squared off by 15:15 IST.
"""
...
class Alpha68NR5HighConvictionGap(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_68_NR5_HIGH_CONVICTION_GAP",
            name="Alpha_68_NR5_High_Conviction_Gap",
            category="VOLATILITY_CONTRACTION_EXPANSION",
            economic_rationale=(
                "NR5 5-session equilibrium combined with extreme 2.25x volume shock on an aligned opening gap "
                "creates high directional edge that clears Indian costs."
            ),
            target_instruments=[
        ...
        for i in range(n):
            ...
            if bar_time >= t_1515:
                if curr_state != 0.0:
                    curr_state = 0.0
                    signals[i] = 0.0
                    rationales[i] = "Alpha 68 EXIT: 15:15 EOD Square-Off"
                continue
            ...
            if bar_time == t_0915:
                ...
                if bar_range > 0 and (body_size / bar_range) >= min_body and rvol >= min_rvol:
                    # Bullish NR5 High-Conviction Gap (LONG)
                    if gap >= min_gap and closes[i] > prev_highs[i] and closes[i] > opens[i]:
                        ...
                        curr_sl = sl
                        curr_tp = tp
                        curr_rationale = f"Alpha 68 LONG: NR5 High-Conviction Gap Close={closes[i]:.1f} > PDH={prev_highs[i]:.1f} | RVOL={rvol:.2f}x"
                        signals[i] = 1.0
                        stop_loss[i] = curr_sl
                        take_profit[i] = curr_tp
                        rationales[i] = curr_rationale
                    ...
                    # Bearish NR5 High-Conviction Gap (SHORT)
                        ...
                        curr_sl = sl
                        curr_tp = tp
                        curr_rationale = f"Alpha 68 SHORT: NR5 High-Conviction Gap Close={closes[i]:.1f} < PDL={prev_lows[i]:.1f} | RVOL={rvol:.2f}x"
                        signals[i] = -1.0
                        stop_loss[i] = curr_sl
                        take_profit[i] = curr_tp
                        rationales[i] = curr_rationale
