"""
Alpha 74: Gap Marubozu Momentum Expansion (ALPHA_74_GAP_MARUBOZU_MOMENTUM)
Identifies extreme directional conviction on opening bar where candle body >= 70% of bar range
accompanied by volume surge and moderate gap.
"""
...
class Alpha74GapMarubozuMomentum(BaseHypothesis):
    """
    Alpha 74: Gap Marubozu Momentum Expansion
    Fires on high conviction Marubozu (Body >= 70%) with moderate gap and volume surge.
    """
    ...
    def __init__(
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_74_GAP_MARUBOZU_MOMENTUM",
            name="Alpha_74_Gap_Marubozu_Momentum",
            category="GAP_MOMENTUM",
            economic_rationale=(
                "Opening Marubozu (Body >= 70%) with gap 0.25%-1.20% and RVOL >= 1.25x."
            ),
            target_instruments=[
            ...
            if bar_time == t_0915:
                ...
                if min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= 0.70:
                    if gap_pct > 0 and closes[i] > opens[i]:
                        ...
                        curr_rationale = f"Alpha74 LONG: Gap Marubozu Body {body_ratio:.2f}, Gap +{gap_pct*100:.2f}%, RVOL {rvol:.1f}x"
                        ...
                        tp = closes[i] - (target_rr * risk)
                        curr_sl, curr_tp = sl, tp
                        curr_rationale = f"Alpha74 SHORT: Gap Marubozu Body {body_ratio:.2f}, Gap {gap_pct*100:.2f}%, RVOL {rvol:.1f}x"
                        signals[i], stop_loss[i], take_profit[i] = -1.0, curr_sl, curr_tp
                        rationales[i] = curr_rationale
                        traded_today = True
