"""
Ashva Quantitative Strategy: High-Conviction Opening Gap Drift (Alpha 46)
Captures persistent institutional trend continuation following extreme volume shock (RVOL >= 2.25x) opening gaps.

Hypothesis:
When an equity opens with a moderate overnight gap (0.35% to 1.50%) and the opening 15m candle exhibits extreme
institutional volume shock (RVOL >= 2.25x) with strong directional body (>= 65%), the sheer scale of the order-flow
imbalance produces sustained trending continuation into a 1.75R target, squared off by 15:15 IST.
"""
...
class Alpha46HighConvictionGapDrift(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_46_HIGH_CONVICTION_GAP_DRIFT",
            name="Alpha_46_High_Conviction_Gap_Drift",
            category="ORDER_FLOW_IMBALANCE",
            economic_rationale=(
                "Extreme volume shock (RVOL >= 2.25x) on opening gap bars indicates major institutional block allocation, "
                "yielding high trending momentum and low adverse excursion."
            ),
            target_instruments=[
        ...
        for i in range(n):
            ...
            if bar_time >= t_1515:
                if curr_state != 0.0:
                    curr_state = 0.0
                    signals[i] = 0.0
                    rationales[i] = "Alpha 46 EXIT: 15:15 EOD Square-Off"
                continue
                ...
                if min_gap <= abs_gap <= max_gap:
                    ...
                    if bar_range > 0 and (body_size / bar_range) >= min_body and rvol >= min_rvol:
                        ...
                        if gap_pct > 0 and closes[i] > opens[i]:
                            ...
                            curr_sl = sl
                            curr_tp = tp
                            curr_rationale = f"Alpha 46 LONG: Extreme Gap RVOL={rvol:.2f}x | Body={body_size/bar_range*100:.0f}%"
                            signals[i] = 1.0
                            stop_loss[i] = curr_sl
                            take_profit[i] = curr_tp
                            rationales[i] = curr_rationale
                        ...
                        # Bearish Gap Continuation (SHORT)
                            ...
                            curr_sl = sl
                            curr_tp = tp
                            curr_rationale = f"Alpha 46 SHORT: Extreme Gap RVOL={rvol:.2f}x | Body={body_size/bar_range*100:.0f}%"
                            signals[i] = -1.0
                            stop_loss[i] = curr_sl
                            take_profit[i] = curr_tp
                            rationales[i] = curr_rationale
