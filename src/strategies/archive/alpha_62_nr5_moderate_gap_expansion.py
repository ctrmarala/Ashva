"""
Ashva Quantitative Strategy: NR5 Moderate Gap Volume Shock Expansion (Alpha 62)
Captures directional intraday trend expansion when a 5-day narrow range (NR5) is resolved by a moderate opening gap on 2.0x volume.

Hypothesis:
When Day T-1 range is the narrowest of the prior 5 sessions (NR5 condition) and Day T opens with a moderate gap
(0.35% to 1.20%) breaking Day T-1 range on 2.0x volume shock, multi-day potential energy releases with high
directional conviction toward a 1.50R target, squared off by 15:15 IST.
"""
...
class Alpha62NR5ModerateGapExpansion(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_62_NR5_MODERATE_GAP_EXPANSION",
            name="Alpha_62_NR5_Moderate_Gap_Expansion",
            category="VOLATILITY_CONTRACTION_EXPANSION",
            economic_rationale=(
                "NR5 range compression combined with a moderate gap and 2.0x volume shock releases multi-session tension "
                "with strong programmatic institutional follow-through."
            ),
            target_instruments=[
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # NR5 calculation from completed prior sessions (Shifted 1 session)
        is_nr5 = pd.Series(False, index=daily_summary.index)
        for i in range(5, len(daily_summary)):
            t_minus_1_range = daily_range.iloc[i - 1]
            prior_4_ranges = daily_range.iloc[i - 5:i - 1]
            if t_minus_1_range < prior_4_ranges.min():
                is_nr5.iloc[i] = True
        ...
        out["is_nr5_day"] = pd.Series(dates, index=out.index).map(is_nr5).ffill().fillna(False)
        ...
        nr5_flags = out["is_nr5_day"].values
        ...
        for i in range(n):
            ...
            if bar_time >= t_1515:
                if curr_state != 0.0:
                    curr_state = 0.0
                    signals[i] = 0.0
                    rationales[i] = "Alpha 62 EXIT: 15:15 EOD Square-Off"
                continue
            ...
            if traded_today or not nr5_flags[i] or pd.isna(prev_closes[i]) or prev_closes[i] <= 0:
                continue
            ...
            if bar_time == t_0915:
                ...
                if min_gap <= abs_gap <= max_gap:
                    ...
                    if bar_range > 0 and (body_size / bar_range) >= min_body and rvol >= min_rvol:
                        # Bullish NR5 Moderate Gap Breakout (LONG)
                        if gap_pct > 0 and closes[i] > prev_highs[i] and closes[i] > opens[i]:
                            ...
                            curr_sl = sl
                            curr_tp = tp
                            curr_rationale = f"Alpha 62 LONG: NR5 Moderate Gap Close={closes[i]:.1f} > PDH={prev_highs[i]:.1f} | RVOL={rvol:.2f}x"
                            signals[i] = 1.0
                            stop_loss[i] = curr_sl
                            take_profit[i] = curr_tp
                            rationales[i] = curr_rationale
                        ...
                        # Bearish NR5 Moderate Gap Breakout (SHORT)
                            ...
                            curr_sl = sl
                            curr_tp = tp
                            curr_rationale = f"Alpha 62 SHORT: NR5 Moderate Gap Close={closes[i]:.1f} < PDL={prev_lows[i]:.1f} | RVOL={rvol:.2f}x"
                            signals[i] = -1.0
                            stop_loss[i] = curr_sl
                            take_profit[i] = curr_tp
                            rationales[i] = curr_rationale
