"""
Ashva Quantitative Strategy: NR4 High-Volume Gap Breakout (Alpha 65)
Captures directional intraday trend expansion when a 4-day narrow range (NR4) is resolved by an opening gap on 2.0x volume.

Hypothesis:
When Day T-1 range is the narrowest of the prior 4 sessions (NR4 condition) and Day T opens with an aligned gap (>= 0.30%)
breaking Day T-1 range on 2.0x volume shock, compression energy releases with strong institutional delta follow-through
toward a 1.75R target, squared off by 15:15 IST.
"""
...
class Alpha65NR4HighVolumeGap(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_65_NR4_HIGH_VOLUME_GAP",
            name="Alpha_65_NR4_High_Volume_Gap",
            category="VOLATILITY_CONTRACTION_EXPANSION",
            economic_rationale=(
                "NR4 4-session range compression combined with 2.0x volume shock on an aligned opening gap triggers "
                "explosive multi-session expansion that easily overcomes statutory costs."
            ),
            target_instruments=[
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # NR4 calculation from completed prior sessions (Shifted 1 session)
        ...
        for i in range(n):
            ...
            if bar_time >= t_1515:
                if curr_state != 0.0:
                    curr_state = 0.0
                    signals[i] = 0.0
                    rationales[i] = "Alpha 65 EXIT: 15:15 EOD Square-Off"
                continue
            ...
            if bar_time == t_0915:
                ...
                if bar_range > 0 and (body_size / bar_range) >= min_body and rvol >= min_rvol:
                    # Bullish NR4 High-Volume Gap Breakout (LONG)
                    if gap >= min_gap and closes[i] > prev_highs[i] and closes[i] > opens[i]:
                        ...
                        curr_sl = sl
                        curr_tp = tp
                        curr_rationale = f"Alpha 65 LONG: NR4 High-Vol Gap Close={closes[i]:.1f} > PDH={prev_highs[i]:.1f} | RVOL={rvol:.2f}x"
                        signals[i] = 1.0
                        stop_loss[i] = curr_sl
                        take_profit[i] = curr_tp
                        rationales[i] = curr_rationale
                    ...
                    # Bearish NR4 High-Volume Gap Breakout (SHORT)
                        ...
                        curr_sl = sl
                        curr_tp = tp
                        curr_rationale = f"Alpha 65 SHORT: NR4 High-Vol Gap Close={closes[i]:.1f} < PDL={prev_lows[i]:.1f} | RVOL={rvol:.2f}x"
                        signals[i] = -1.0
                        stop_loss[i] = curr_sl
                        take_profit[i] = curr_tp
                        rationales[i] = curr_rationale
