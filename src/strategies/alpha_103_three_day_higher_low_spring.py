"""
Ashva Quantitative Strategy: 3-Day Higher-Low Compression Spring (Alpha 103)
Category: THREE_DAY_HIGHER_LOW_SPRING
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When an equity exhibits 3 consecutive days of ascending daily lows (Low[T-1] > Low[T-2] > Low[T-3] for Long,
or High[T-1] < High[T-2] < High[T-3] for Short) with tight volatility compression on Day T-1 (Range[T-1] <= 0.70x ATR)
aligned above the 50 SMA (below 50 SMA for Short), dip-buyers are aggressively defending higher price levels without letting price pull back.
A confirming morning opening breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x sparks an explosive upward continuation surge.
"""
...
class Alpha103ThreeDayHigherLowSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "103_alpha"
    hypothesis_id = "103_alpha"
    name = "103_alpha - 3-Day Higher-Low Compression Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="THREE_DAY_HIGHER_LOW_SPRING",
            economic_rationale=(
                "Consecutive ascending lows (Low[T-1] > Low[T-2] > Low[T-3]) accompanied by <= 0.70x ATR compression "
                "demonstrates persistent institutional bid support, creating high-expectancy morning breakout continuation."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="103_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_bull_setup = (l1 > l2) & (l2 > l3) & (c1 > sma50) & (rng1 <= (atr14 * max_rng_ratio))
        is_bear_setup = (h1 < h2) & (h2 < h3) & (c1 < sma50) & (rng1 <= (atr14 * max_rng_ratio))
