"""
Ashva Quantitative Strategy: Inside Day Compression Breakout (Alpha 101)
Category: INSIDE_DAY_COMPRESSION
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When an equity forms an Inside Day on Day T-1 (High[T-1] < High[T-2] and Low[T-1] > Low[T-2])
with tight volatility compression (Range[T-1] <= 0.70x ATR) and trend alignment (Close[T-1] > 50 SMA for Long, or < 50 SMA for Short),
market liquidity is coiled inside the mother bar with zero directional progress.
A confirming morning opening breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x triggers an asymmetric expansion out of the inside range.
"""
...
class Alpha101InsideDayCompressionBreakout(BaseHypothesis, BaseStrategy):
    strategy_id = "101_alpha"
    hypothesis_id = "101_alpha"
    name = "101_alpha - Inside Day Compression Breakout"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="INSIDE_DAY_COMPRESSION",
            economic_rationale=(
                "Inside day compression (High < High[T-2] and Low > Low[T-2]) within <= 0.70x ATR indicates supply-demand "
                "equilibrium before a powerful morning volatility expansion in the trend direction."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="101_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_inside = (h1 < h2) & (l1 > l2) & (rng1 <= (atr14 * max_rng_ratio))
        is_bull_setup = is_inside & (c1 > sma50)
        is_bear_setup = is_inside & (c1 < sma50)
