"""
Ashva Quantitative Strategy: NR3 Inside Volatility Spring (Alpha 58)
Category: VOLATILITY_COMPRESSION_SPRING
Market Mechanism: BREAKOUT

Hypothesis:
When Day T-1 forms an Inside Day whose range is also narrower than Day T-2 and Day T-3 (Narrow Range 3 - NR3),
volatility compression is sustained across a multi-session window.
Day T morning 15m auction breakout provides high-probability trend expansion.
"""
...
class Alpha58NR3InsideSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "58_alpha"
    hypothesis_id = "58_alpha"
    name = "58_alpha — NR3 Inside Volatility Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="VOLATILITY_COMPRESSION_SPRING",
            economic_rationale=(
                "Inside Day with NR3 range compression represents multi-session equilibrium. "
                "The morning breakout impulse triggers rapid trend expansion."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="58_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_nr3 = (rng.shift(1) < rng.shift(2)) & (rng.shift(1) < rng.shift(3))
        is_spring = is_id & is_nr3
