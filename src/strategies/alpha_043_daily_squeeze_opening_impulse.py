"""
Ashva Quantitative Strategy: Daily Squeeze Opening Impulse (Alpha 43)
Category: VOLATILITY_SQUEEZE_EXPANSION
Market Mechanism: BREAKOUT

Hypothesis:
When an equity's daily Bollinger Bands (20, 2.0) compress entirely inside its Keltner Channels (20, 1.5 ATR),
a master volatility squeeze is established. An opening 15m breakout on Day T with relative volume shock (RVOL >= 1.25x)
and directional close unleashes explosive institutional momentum with high win rate and low tail drawdown.
"""
...
class Alpha43DailySqueezeOpeningImpulse(BaseHypothesis, BaseStrategy):
    strategy_id = "43_alpha"
    hypothesis_id = "43_alpha"
    name = "43_alpha — Daily Squeeze Opening Impulse"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="VOLATILITY_SQUEEZE_EXPANSION",
            economic_rationale=(
                "Daily Bollinger Band contraction inside Keltner Channels marks institutional volatility coiling. "
                "The opening volume impulse provides the directional catalyst for sustained intraday trend expansion."
            ),
            target_instruments=[],
            timeframe="15m",
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Daily Keltner Channels (20, 1.5 ATR)
        rng = h - l
        atr20 = rng.rolling(20, min_periods=15).mean()
        kc_upper = sma20 + (1.5 * atr20)
        kc_lower = sma20 - (1.5 * atr20)

        # Volatility Squeeze condition on Day T-1: BB inside KC
        is_squeeze = (bb_upper < kc_upper) & (bb_lower > kc_lower)
        is_squeeze_t1 = is_squeeze.shift(1).fillna(False)

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_squeeze"] = pd.Series(dates, index=out.index).map(is_squeeze_t1).ffill().fillna(False)
        ...
        squeeze_flags = out["is_squeeze"].values
