"""
Ashva Quantitative Strategy: Consecutive Volume Contraction Spring (Alpha 51)
Category: MULTI_DAY_VOLUME_EXHAUSTION_SPRING
Market Mechanism: BREAKOUT

Hypothesis:
When an equity forms an Inside Day on Day T-1 while volume has contracted for two consecutive sessions
(Volume T-1 < Volume T-2 < Volume T-3), order flow reaches complete liquidity exhaustion.
An opening 15m breakout drive on Day T with institutional volume shock (RVOL >= 1.20x) and strong directional candle body (>= 0.60)
triggers aggressive momentum expansion with high win rate and superior post-tax profit factor.
"""
...
class Alpha51ConsecutiveVolumeContractionSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "51_alpha"
    hypothesis_id = "51_alpha"
    name = "51_alpha — Consecutive Volume Contraction Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="MULTI_DAY_VOLUME_EXHAUSTION_SPRING",
            economic_rationale=(
                "Two consecutive days of volume drying up combined with an Inside Day creates severe supply exhaustion. "
                "The opening volume catalyst triggers rapid trend expansion as market orders sweep the empty book."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="51_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 2 Consecutive Days of Volume Contraction: V(T-1) < V(T-2) AND V(T-2) < V(T-3)
        is_vol_decay = (v.shift(1) < v.shift(2)) & (v.shift(2) < v.shift(3))

        # Inside Day on Day T-1: H(T-1) < H(T-2) AND L(T-1) > L(T-2)
        is_id = (h.shift(1) < h.shift(2)) & (l.shift(1) > l.shift(2))
        is_exhaustion_spring = is_id & is_vol_decay

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_exhaustion_spring"] = pd.Series(dates, index=out.index).map(is_exhaustion_spring).ffill().fillna(False)
        ...
        spring_flags = out["is_exhaustion_spring"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if spring_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
