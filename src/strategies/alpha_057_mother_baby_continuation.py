"""
Ashva Quantitative Strategy: Mother-Baby Expansion-Contraction Continuation (Alpha 57)
Category: EXPANSION_CONTRACTION_CYCLES
Market Mechanism: BREAKOUT

Hypothesis:
When Day T-2 exhibits an aggressive directional expansion trend (Range >= 80% ATR10), and Day T-1 forms an Inside Day
(contained entirely within Day T-2 without retracing the trend), institutional inventory is in consolidation.
Day T morning auction breakout in the direction of the Day T-2 trend produces high-velocity continuation.
"""
...
class Alpha57MotherBabyContinuation(BaseHypothesis, BaseStrategy):
    strategy_id = "57_alpha"
    hypothesis_id = "57_alpha"
    name = "57_alpha — Mother-Baby Expansion-Contraction Continuation"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="EXPANSION_CONTRACTION_CYCLES",
            economic_rationale=(
                "Expansion trend on Day T-2 followed by tight Inside Day pause on Day T-1 sets up "
                "directional trend continuation upon Day T morning volume impulse."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="57_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        atr10 = rng.shift(3).rolling(10, min_periods=8).mean()
        is_id = (h.shift(1) < h.shift(2)) & (l.shift(1) > l.shift(2))
        t2_expanded = rng.shift(2) > (0.80 * atr10)
        t2_bull = c.shift(2) > o.shift(2)
        t2_bear = c.shift(2) < o.shift(2)

        cond_long = is_id & t2_expanded & t2_bull
        cond_short = is_id & t2_expanded & t2_bear
