"""
Ashva Quantitative Strategy: Sub-0.70 ATR Trend Compression Spring (Alpha 65)
Category: ULTRA_COMPRESSION_TREND_SPRING
Market Mechanism: BREAKOUT

Hypothesis:
When an equity is in a confirmed 50 SMA and 20 EMA trend alignment and experiences extreme daily
volatility compression (Daily Range <= 0.70x 14-day ATR), elastic potential energy reaches maximum
concentration. On Day T, morning 15m opening gap impulse unlocks sustained directional momentum.
"""
...
class Alpha65Sub70ATRTrendCompressionSpring(BaseHypothesis, BaseStrategy):
    strategy_id = "65_alpha"
    hypothesis_id = "65_alpha"
    name = "65_alpha — Sub-0.70 ATR Trend Compression Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="ULTRA_COMPRESSION_TREND_SPRING",
            economic_rationale=(
                "Ultra-compression (sub-0.70x ATR) in confirmed 50 SMA / 20 EMA trend provides "
                "asymmetric low-risk, high-reward morning breakout expansion."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="65_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_ultra_compressed = rng.shift(1) <= (0.70 * atr14)
        bull_trend_spring = (c.shift(1) > sma50) & (c.shift(1) > ema20) & is_ultra_compressed
        bear_trend_spring = (c.shift(1) < sma50) & (c.shift(1) < ema20) & is_ultra_compressed
