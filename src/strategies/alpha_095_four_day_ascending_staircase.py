"""
Ashva Quantitative Strategy: 4-Day Ascending / Descending Structural Staircase (Alpha 95)
Category: STRUCTURAL_STAIRCASE_EXPANSION
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity prints 4 consecutive sessions of ascending daily lows (Low[T-1] > Low[T-2] > Low[T-3] > Low[T-4] for Long,
or High[T-1] < High[T-2] < High[T-3] < High[T-4] for Short) with orderly volatility (Range[T-1] <= 0.75x ATR)
and 50 SMA trend alignment (Close[T-1] > 50 SMA for Long, or < 50 SMA for Short),
institutional limit orders are stepping up at successively higher inflection points.
...
class Alpha95FourDayAscendingStaircase(BaseHypothesis, BaseStrategy):
    strategy_id = "95_alpha"
    hypothesis_id = "95_alpha"
    name = "95_alpha - 4-Day Ascending Structural Staircase"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="STRUCTURAL_STAIRCASE_EXPANSION",
            economic_rationale=(
                "Four consecutive ascending lows with controlled range (<= 0.75x ATR) and 50 SMA alignment "
                "proves persistent institutional demand stepping up across the entire trading week."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="95_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_4d_staircase_bull = (l1 > l2) & (l2 > l3) & (l3 > l4) & (c1 > sma50)
        is_4d_staircase_bear = (h1 < h2) & (h2 < h3) & (h3 < h4) & (c1 < sma50)
        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.75))
        is_orderly = (rng1 <= (atr14 * max_rng_ratio))

        is_bull_trend_map = (is_4d_staircase_bull & is_orderly).to_dict()
        is_bear_trend_map = (is_4d_staircase_bear & is_orderly).to_dict()
