"""
Ashva Quantitative Strategy: Multi-Day Trend Surge with Opening Drive (Alpha 37)
Category: TREND_MOMENTUM
Market Mechanism: MOMENTUM

Hypothesis:
When an equity demonstrates consecutive 2-day upward closes with expanding volume (institutional accumulation),
an opening gap above Day T-1 high with morning Relative Volume (RVOL >= 1.50x) triggers high-conviction
momentum continuation. Using a structural 15m candle stop and 1:1.50 risk-reward delivers robust post-tax alpha.
"""
...
class Alpha37MultiDayTrendOpeningDrive(BaseHypothesis, BaseStrategy):
    strategy_id = "37_alpha"
    hypothesis_id = "37_alpha"
    name = "37_alpha — Multi-Day Trend Surge with Opening Drive"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="TREND_MOMENTUM",
            economic_rationale=(
                "Two consecutive bullish daily closes with volume expansion signify institutional accumulation. "
                "An opening gap above the previous day high with high morning volume generates persistent momentum."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="37_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Multi-day Bullish Trend condition on T-1:
        # Day T-1 close > Day T-1 open, Day T-2 close > Day T-2 open, Day T-1 close > Day T-2 close
        is_bull_trend = (c.shift(1) > o.shift(1)) & (c.shift(2) > o.shift(2)) & (c.shift(1) > c.shift(2)) & (v.shift(1) > v.shift(2))
        is_bear_trend = (c.shift(1) < o.shift(1)) & (c.shift(2) < o.shift(2)) & (c.shift(1) < c.shift(2)) & (v.shift(1) > v.shift(2))
        
        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_bull_trend"] = pd.Series(dates, index=out.index).map(is_bull_trend).ffill().fillna(False)
        out["is_bear_trend"] = pd.Series(dates, index=out.index).map(is_bear_trend).ffill().fillna(False)
        ...
        bull_trend_flags = out["is_bull_trend"].values
        bear_trend_flags = out["is_bear_trend"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
                    if bull_trend_flags[i] and gap_pct > 0 and closes[i] > prev_highs[i] and closes[i] > opens[i]:
                        sl = lows[i]
                        ...
                        traded_today = True
                    elif bear_trend_flags[i] and gap_pct < 0 and closes[i] < prev_lows[i] and closes[i] < opens[i]:
                        sl = highs[i]
