"""
Ashva Quantitative Strategy: NR7 Volatility Compression Overnight Gap Breakout (Alpha 48)
Captures directional intraday trend expansion when a 7-day narrow range (NR7) is resolved by an aligned opening gap.

Hypothesis:
When Day T-1 range is the narrowest of the prior 7 sessions (NR7 condition), multi-day volatility compression
reaches absolute maximum potential energy. On Day T, an overnight gap (>= 0.30%) breaking Day T-1 high/low
with volume shock (RVOL >= 1.40x) triggers massive institutional delta follow-through toward a 1.75R target,
...
class Alpha48NR7GapBreakout(BaseHypothesis):
    def __init__(
        self,
        ...
        meta = metadata or HypothesisMetadata(
            hypothesis_id="HYP_ALPHA_48_NR7_GAP_BREAKOUT",
            name="Alpha_48_NR7_Gap_Breakout",
            category="VOLATILITY_CONTRACTION_EXPANSION",
            economic_rationale=(
                "NR7 daily range compression represents 7-session market balance. An aligned opening gap breaking "
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # NR7 calculation strictly from completed prior sessions (Shifted 1 session)
        ...
        for i in range(n):
            ...
            if bar_time >= t_1515:
                if curr_state != 0.0:
                    curr_state = 0.0
                    signals[i] = 0.0
                    rationales[i] = "Alpha 48 EXIT: 15:15 EOD Square-Off"
                continue
            ...
            if bar_time == t_0915:
                ...
                if gap >= min_gap and closes[i] > prev_highs[i] and closes[i] > opens[i] and rvol >= min_rvol:
                    ...
                    curr_sl = sl
                    curr_tp = tp
                    curr_rationale = f"Alpha 48 LONG: NR7 Gap Breakout Close={closes[i]:.1f} > PDH={prev_highs[i]:.1f} | RVOL={rvol:.2f}x"
                    signals[i] = 1.0
                    stop_loss[i] = curr_sl
                    take_profit[i] = curr_tp
                    rationales[i] = curr_rationale
                ...
                # Bearish NR7 Gap Breakout (SHORT)
                    ...
                    curr_sl = sl
                    curr_tp = tp
                    curr_rationale = f"Alpha 48 SHORT: NR7 Gap Breakout Close={closes[i]:.1f} < PDL={prev_lows[i]:.1f} | RVOL={rvol:.2f}x"
                    signals[i] = -1.0
                    stop_loss[i] = curr_sl
                    take_profit[i] = curr_tp
                    rationales[i] = curr_rationale
