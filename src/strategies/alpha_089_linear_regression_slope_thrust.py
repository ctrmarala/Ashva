"""
Ashva Quantitative Strategy: 5-Day Linear Regression Slope Trend Thrust (Alpha 89)
Category: LINEAR_REGRESSION_SLOPE_THRUST
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity exhibits a verified positive linear regression slope over the past 5 sessions
(Slope5 >= +0.20 normalized by ATR for Long, or Slope5 <= -0.20 for Short) aligned with 50 SMA (Close[T-1] > 50 SMA)
and orderly volatility (Range[T-1] <= 0.75x ATR), institutional drift momentum carries strongly into Day T.
A confirming morning directional breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x triggers high-expectancy intraday continuation.
"""
...
class Alpha89LinearRegressionSlopeThrust(BaseHypothesis, BaseStrategy):
    strategy_id = "89_alpha"
    hypothesis_id = "89_alpha"
    name = "89_alpha - 5-Day Linear Regression Slope Trend Thrust"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="LINEAR_REGRESSION_SLOPE_THRUST",
            economic_rationale=(
                "Positive 5-day linear regression slope with controlled range (<= 0.75x ATR) and 50 SMA alignment "
                "identifies stable institutional accumulation that expands rapidly on Day T morning gap breakouts."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="89_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 5-day linear regression slope calculation on daily closes (strictly shifted by 1 day)
        # x = [0, 1, 2, 3, 4], mean(x) = 2, sum((x - 2)^2) = 10
        # slope = sum((x_i - 2) * (c_{T-5+i})) / 10
        c_t1 = c.shift(1)
        c_t2 = c.shift(2)
        c_t3 = c.shift(3)
        c_t4 = c.shift(4)
        ...
        slope_5d = (-2.0 * c_t5 - 1.0 * c_t4 + 0.0 * c_t3 + 1.0 * c_t2 + 2.0 * c_t1) / 10.0
        norm_slope = slope_5d / np.maximum(atr14, 1.0)

        is_slope_bull = (norm_slope >= 0.15) & (c1 > sma50)
        is_slope_bear = (norm_slope <= -0.15) & (c1 < sma50)
        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.75))
        is_orderly = (rng1 <= (atr14 * max_rng_ratio))

        is_bull_trend_map = (is_slope_bull & is_orderly).to_dict()
        is_bear_trend_map = (is_slope_bear & is_orderly).to_dict()
