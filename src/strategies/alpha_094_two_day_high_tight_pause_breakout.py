"""
Ashva Quantitative Strategy: 2-Day High-Tight Pause-Day Breakout Surge (Alpha 94)
Category: HIGH_TIGHT_PAUSE_BREAKOUT
Market Mechanism: MOMENTUM_EXPANSION

Hypothesis:
When a liquid equity exhibits an expansive directional thrust on Day T-2 (Close[T-2] > Close[T-3] for Long, or < Close[T-3] for Short)
followed by an orderly pause/digestion day on Day T-1 (Range[T-1] <= 0.70x ATR) that holds the directional gains
(Close[T-1] >= Close[T-2] for Long, or Close[T-1] <= Close[T-2] for Short) above the 50 SMA,
institutional buyers are absorbing supply at the highs.
A confirming morning directional breakout (09:15-09:30 IST) with gap >= 0.35%, solid candle body >= 65%,
and RVOL >= 1.25x triggers the next explosive expansion leg.
"""
...
class Alpha94TwoDayHighTightPauseBreakout(BaseHypothesis, BaseStrategy):
    strategy_id = "94_alpha"
    hypothesis_id = "94_alpha"
    name = "94_alpha - 2-Day High-Tight Pause-Day Breakout Surge"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="HIGH_TIGHT_PAUSE_BREAKOUT",
            economic_rationale=(
                "Holding gains on a narrow-range pause day (<= 0.70x ATR) following an expansion thrust demonstrates "
                "supply exhaustion at the highs, creating high-expectancy continuation on Day T morning breakouts."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="94_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        is_bull_pause = (c2 > c3) & (c1 >= c2) & (c1 > sma50)
        is_bear_pause = (c2 < c3) & (c1 <= c2) & (c1 < sma50)
        max_rng_ratio = float(self.parameters.get("max_range_atr", 0.70))
        is_orderly = (rng1 <= (atr14 * max_rng_ratio))

        is_bull_trend_map = (is_bull_pause & is_orderly).to_dict()
        is_bear_trend_map = (is_bear_pause & is_orderly).to_dict()
