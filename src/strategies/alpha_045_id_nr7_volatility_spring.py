"""
Ashva Quantitative Strategy: Inside Day NR7 Volatility Spring (Alpha 45)
Category: DUAL_VOLATILITY_COMPRESSION_SPRING
Market Mechanism: BREAKOUT

Hypothesis:
When an equity forms an Inside Day that is simultaneously an NR7 (the narrowest range of the past 7 days),
volatility compression reaches its theoretical maximum entropy. An opening 15m directional drive on Day T with
RVOL >= 1.20x and body ratio >= 0.60 releases stored institutional energy, producing superior win rates (>= 60%),
exceptional profit factor (>= 1.50), and low tail risk.
"""
...
class Alpha45IDNR7VolatilitySpring(BaseHypothesis, BaseStrategy):
    strategy_id = "45_alpha"
    hypothesis_id = "45_alpha"
    name = "45_alpha — Inside Day NR7 Volatility Spring"
    ...
    def __init__(self, parameters: Optional[Dict[str, Any]] = None):
        ...
        metadata = HypothesisMetadata(
            ...
            category="DUAL_VOLATILITY_COMPRESSION_SPRING",
            economic_rationale=(
                "Inside Day coinciding with the 7-day narrowest range (ID-NR7) captures maximum volatility coiling. "
                "The subsequent opening volume shock provides explosive directional follow-through."
            ),
            target_instruments=[],
            timeframe="15m",
        ...
        BaseStrategy.__init__(self, strategy_id="45_alpha", parameters=merged)
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # NR7 on Day T-1: Range(T-1) < Range(T-k) for k=2..7
        min_prev_6 = rng.shift(2).rolling(6, min_periods=6).min()
        is_nr7 = rng.shift(1) < min_prev_6

        # Inside Day on Day T-1
        is_id = (h.shift(1) < h.shift(2)) & (l.shift(1) > l.shift(2))

        # ID-NR7 dual condition
        is_id_nr7 = is_id & is_nr7

        prev_close = c.shift(1)
        prev_high = h.shift(1)
        prev_low = l.shift(1)

        out["is_id_nr7"] = pd.Series(dates, index=out.index).map(is_id_nr7).ffill().fillna(False)
        ...
        id_nr7_flags = out["is_id_nr7"].values
        ...
        for i in range(n):
            ...
            if bar_time == t_0915:
                ...
                if id_nr7_flags[i] and min_gap <= abs_gap <= max_gap and rvol >= min_rvol and body_ratio >= min_body:
